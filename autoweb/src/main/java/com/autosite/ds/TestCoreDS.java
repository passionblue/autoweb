...
public class TestCoreDS extends AbstractDS implements DomainStore {
    ...
    private static Logger m_logger = Logger.getLogger(TestCoreDS.class);
	public static boolean m_debug = AutositeGlobals.m_debug;

    private static TestCoreDS m_TestCoreDS = new TestCoreDSExtent();



    public static TestCoreDS getInstance() {
        return m_TestCoreDS;
    }

    public static synchronized TestCoreDS getInstance(long id) {
        TestCoreDS ret = (TestCoreDS) m_dsMap.get(new Long(id));
        if (ret == null) {
            ret = new TestCoreDS(id);
    ...
    public void loadFromDB() throws Exception {
        ...
        m_logger.info("TestCore loaded from DB. num=" + m_idToMap.size());
        
    }


    protected TestCoreDS() {
    ...
    public TestCore getById(Long id) {
        TestCore ret = (TestCore) m_idToMap.get(id);
    ...
    public TestCore getById(Long id, boolean forSynchWithDB) {
        if (persistEnable() && forSynchWithDB ) {
            try {
                TestCore loaded = (TestCore) synchFromDB(id);
                return loaded;
            }
            catch (Exception e) {
                m_logger.error("",e);
        ...
        return (TestCore) m_idToMap.get(id);
    }
    
    public String getEntityClass(){
        return TestCore.class.getName();
    }
    ...
    public void updateMaps(Object obj, boolean del) {
        if (!m_cacheEnable) return;
        TestCore o = (TestCore)obj;
        
        if (del) {
            m_idToMap.remove(new Long(o.getId()));
            if (m_debug) m_logger.debug("TestCore removed from DS " + o.getId());
        }
        else {
            m_idToMap.put(new Long(o.getId()), o);
            if (m_debug) m_logger.debug("TestCore added to DS " + o.getId());
        }
    ...
    private void updateSiteIdMap(Object obj, boolean del) {
        ...
        if (del) {
            ...
            if (m_debug) m_logger.debug("TestCore removed from SiteIdToMap" + o.getId() + " from " + o.getSiteId());
        }
        else {
            ...
            if (m_debug) m_logger.debug("TestCore added to SiteIdToMap " + o.getId() + " to " + o.getSiteId());
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        TestCoreDS ds = new TestCoreDS();
        TestCore obj = ds.getById((long)1);
    ...
    public static TestCore createDefault(){
        ...
        TestCore _TestCore = new TestCore();        
	...
	public String objectToString2(AutositeDataObject object){
		TestCore testCore = (TestCore)object;
		StringBuilder buf = new StringBuilder();
        buf.append("TestCore=");
		buf.append("Id=").append(testCore.getId()).append(", ");
		buf.append("SiteId=").append(testCore.getSiteId()).append(", ");
