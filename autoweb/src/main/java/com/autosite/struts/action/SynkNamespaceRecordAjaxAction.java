...
import com.autosite.holder.SynkNamespaceRecordDataHolder;


public class SynkNamespaceRecordAjaxAction extends SynkNamespaceRecordAction {

    private static Logger m_logger = Logger.getLogger( SynkNamespaceRecordAjaxAction.class);

    public SynkNamespaceRecordAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        SynkNamespaceRecord target = null;
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (SynkNamespaceRecord) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = SynkNamespaceRecordDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            SynkNamespaceRecord _SynkNamespaceRecord = SynkNamespaceRecordDS.getInstance().getById(id);
            if (_SynkNamespaceRecord == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _SynkNamespaceRecord);
                if (field != null)
            ...
            if ( _SynkNamespaceRecord == null) {
                ...
                String field = getFieldByName(fieldName, _SynkNamespaceRecord);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"synkNamespaceRecord-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                SynkNamespaceRecord _SynkNamespaceRecord = (SynkNamespaceRecord) iterator.next();

                buf.append("<div id=\"synkNamespaceRecord-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("namespace")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-namespace\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getNamespace()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("recordId")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-recordId\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getRecordId()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("stamp")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-stamp\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getStamp()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("orgStamp")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-orgStamp\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getOrgStamp()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getTimeCreated()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeUpdated")) {
                    buf.append("<div id=\"synkNamespaceRecord-ajax-timeUpdated\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNamespaceRecord.getTimeUpdated()));
                    buf.append("</div>");
            ...
            List fieldsTitle = SynkNamespaceRecordDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                SynkNamespaceRecordDataHolder _SynkNamespaceRecord  = new SynkNamespaceRecordDataHolder( (SynkNamespaceRecord) iterator.next());
                gen.addTableRow(_SynkNamespaceRecord);
			}
            ...
            buf.append("sendFormAjax('/synkNamespaceRecordAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/synkNamespaceRecordAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            if ( ignoreFieldSet || fieldSet.contains("recordId")) {
                String value = WebUtil.display(request.getParameter("recordId"));

                if ( forceHiddenSet.contains("recordId")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"recordId\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Record Id</div>");
            buf.append("<INPUT NAME=\"recordId\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            if ( ignoreFieldSet || fieldSet.contains("orgStamp")) {
                String value = WebUtil.display(request.getParameter("orgStamp"));

                if ( forceHiddenSet.contains("orgStamp")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"orgStamp\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Org Stamp</div>");
            buf.append("<INPUT NAME=\"orgStamp\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_synkNamespaceRecord(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplaySynkNamespaceRecord\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_synkNamespaceRecord(){\n";
            importedScripts +=     "xmlhttpPostXX('synkNamespaceRecordFormAddDis','/synkNamespaceRecordAction.html', 'resultDisplaySynkNamespaceRecord', '${ajax_response_fields}', responseCallback_synkNamespaceRecord);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_synkNamespaceRecord(){\n";
            importedScripts +=     "clearFormXX('synkNamespaceRecordFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_synkNamespaceRecord(){\n";
            importedScripts +=     "backToXX('synkNamespaceRecordFormAddDis','resultDisplaySynkNamespaceRecord');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"synkNamespaceRecordFormAddDis\" method=\"POST\" action=\"/synkNamespaceRecordAction.html\" id=\"synkNamespaceRecordFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Record Id</div>");
        buf.append("<input class=\"field\" id=\"recordId\" type=\"text\" size=\"70\" name=\"recordId\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Org Stamp</div>");
        buf.append("<input class=\"field\" id=\"orgStamp\" type=\"text\" size=\"70\" name=\"orgStamp\" value=\"\"/> <span></span>");
		buf.append("<br/>");


			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_synkNamespaceRecord()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_synkNamespaceRecord()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplaySynkNamespaceRecord\"></span>");
			buf.append("<a href=\"javascript:showform_synkNamespaceRecord()\">Show form</a><br>");
	        buf.append("');");
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = SynkNamespaceRecordDataHolder.convertToJSON(_SynkNamespaceRecord, fieldSet, ignoreFieldSet, true, true);

                    if ( isSynchRequired() ){
                        try {
                            synchLedger = findOrRegisterSynchLedger(request, _SynkNamespaceRecord.getId(), "cleaner-ticket", true );
                            if ( synchLedger != null) {
                ...
                SynkNamespaceRecord _SynkNamespaceRecord = list.size() >=1?(SynkNamespaceRecord) list.get(0): null; 

					top = SynkNamespaceRecordDataHolder.convertToJSON(_SynkNamespaceRecord, null, false, true, true);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                SynkNamespaceRecord  _SynkNamespaceRecord = SynkNamespaceRecordDS.getInstance().getById(synchTrackingEntry.getObjectId());
                ...
                if (  _SynkNamespaceRecord != null) {
    				totalCount++;
                    JSONObject json = SynkNamespaceRecordDataHolder.convertToJSON(_SynkNamespaceRecord, null, false, true, true);
    ...
    protected List prepareReturnData(HttpServletRequest request, SynkNamespaceRecord target){
    ...
    protected List prepareReturnData(HttpServletRequest request, SynkNamespaceRecord target, boolean isList){
        ...
        if (isList) {
            ...
            List list = SynkNamespaceRecordDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                _SynkNamespaceRecord = target;
            } else if (arg.equals("last")) {
                if (list.size() > 0)
                    _SynkNamespaceRecord = (SynkNamespaceRecord) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _SynkNamespaceRecord = (SynkNamespaceRecord) list.get(0);
            } else {
