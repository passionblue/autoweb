...
import com.autosite.holder.SynkNodeTrackerTxDataHolder;


public class SynkNodeTrackerTxAjaxAction extends SynkNodeTrackerTxAction {

    private static Logger m_logger = Logger.getLogger( SynkNodeTrackerTxAjaxAction.class);

    public SynkNodeTrackerTxAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (SynkNodeTrackerTx) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = SynkNodeTrackerTxDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            SynkNodeTrackerTx _SynkNodeTrackerTx = SynkNodeTrackerTxDS.getInstance().getById(id);
            if (_SynkNodeTrackerTx == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _SynkNodeTrackerTx);
                if (field != null)
            ...
            if ( _SynkNodeTrackerTx == null) {
                ...
                String field = getFieldByName(fieldName, _SynkNodeTrackerTx);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"synkNodeTrackerTx-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                SynkNodeTrackerTx _SynkNodeTrackerTx = (SynkNodeTrackerTx) iterator.next();

                buf.append("<div id=\"synkNodeTrackerTx-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("namespace")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-namespace\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getNamespace()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("deviceId")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-deviceId\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getDeviceId()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("txToken")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-txToken\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getTxToken()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("stampAcked")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-stampAcked\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getStampAcked()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("stampLast")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-stampLast\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getStampLast()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("numRecords")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-numRecords\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getNumRecords()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("ip")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-ip\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getIp()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"synkNodeTrackerTx-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_SynkNodeTrackerTx.getTimeCreated()));
                    buf.append("</div>");
            ...
            if ( ignoreFieldSet || fieldSet.contains("stampAcked")) {
	            buf.append("<td "+ tdBoldStyleStr +" >");
	            buf.append("Stamp Acked");
	            buf.append("</td>");
			}
            if ( ignoreFieldSet || fieldSet.contains("stampLast")) {
	            buf.append("<td "+ tdBoldStyleStr +" >");
	            buf.append("Stamp Last");
	            buf.append("</td>");
			}
            if ( ignoreFieldSet || fieldSet.contains("numRecords")) {
	            buf.append("<td "+ tdBoldStyleStr +" >");
	            buf.append("Num Records");
	            buf.append("</td>");
			}
            if ( ignoreFieldSet || fieldSet.contains("ip")) {
	            buf.append("<td "+ tdBoldStyleStr +" >");
            ...
            List fieldsTitle = SynkNodeTrackerTxDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                SynkNodeTrackerTxDataHolder _SynkNodeTrackerTx  = new SynkNodeTrackerTxDataHolder( (SynkNodeTrackerTx) iterator.next());
                gen.addTableRow(_SynkNodeTrackerTx);
			}
            ...
            buf.append("sendFormAjax('/synkNodeTrackerTxAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/synkNodeTrackerTxAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            if ( ignoreFieldSet || fieldSet.contains("txToken")) {
                String value = WebUtil.display(request.getParameter("txToken"));

                if ( forceHiddenSet.contains("txToken")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"txToken\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Tx Token</div>");
            buf.append("<INPUT NAME=\"txToken\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}

            if ( ignoreFieldSet || fieldSet.contains("stampAcked")) {
                String value = WebUtil.display(request.getParameter("stampAcked"));

                if ( forceHiddenSet.contains("stampAcked")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"stampAcked\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Stamp Acked</div>");
            buf.append("<INPUT NAME=\"stampAcked\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}

            if ( ignoreFieldSet || fieldSet.contains("stampLast")) {
                String value = WebUtil.display(request.getParameter("stampLast"));

                if ( forceHiddenSet.contains("stampLast")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"stampLast\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Stamp Last</div>");
            buf.append("<INPUT NAME=\"stampLast\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}

            if ( ignoreFieldSet || fieldSet.contains("numRecords")) {
                String value = WebUtil.display(request.getParameter("numRecords"));

                if ( forceHiddenSet.contains("numRecords")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"numRecords\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Num Records</div>");
            buf.append("<INPUT NAME=\"numRecords\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_synkNodeTrackerTx(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplaySynkNodeTrackerTx\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_synkNodeTrackerTx(){\n";
            importedScripts +=     "xmlhttpPostXX('synkNodeTrackerTxFormAddDis','/synkNodeTrackerTxAction.html', 'resultDisplaySynkNodeTrackerTx', '${ajax_response_fields}', responseCallback_synkNodeTrackerTx);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_synkNodeTrackerTx(){\n";
            importedScripts +=     "clearFormXX('synkNodeTrackerTxFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_synkNodeTrackerTx(){\n";
            importedScripts +=     "backToXX('synkNodeTrackerTxFormAddDis','resultDisplaySynkNodeTrackerTx');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"synkNodeTrackerTxFormAddDis\" method=\"POST\" action=\"/synkNodeTrackerTxAction.html\" id=\"synkNodeTrackerTxFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Tx Token</div>");
        buf.append("<input class=\"field\" id=\"txToken\" type=\"text\" size=\"70\" name=\"txToken\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Stamp Acked</div>");
        buf.append("<input class=\"field\" id=\"stampAcked\" type=\"text\" size=\"70\" name=\"stampAcked\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Stamp Last</div>");
        buf.append("<input class=\"field\" id=\"stampLast\" type=\"text\" size=\"70\" name=\"stampLast\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Num Records</div>");
        buf.append("<input class=\"field\" id=\"numRecords\" type=\"text\" size=\"70\" name=\"numRecords\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Ip</div>");
        buf.append("<input class=\"field\" id=\"ip\" type=\"text\" size=\"70\" name=\"ip\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_synkNodeTrackerTx()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_synkNodeTrackerTx()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplaySynkNodeTrackerTx\"></span>");
			buf.append("<a href=\"javascript:showform_synkNodeTrackerTx()\">Show form</a><br>");
	        buf.append("');");
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = SynkNodeTrackerTxDataHolder.convertToJSON(_SynkNodeTrackerTx, fieldSet, ignoreFieldSet, true, true);

                    if ( isSynchRequired() ){
                        try {
                            synchLedger = findOrRegisterSynchLedger(request, _SynkNodeTrackerTx.getId(), "cleaner-ticket", true );
                            if ( synchLedger != null) {
                ...
                SynkNodeTrackerTx _SynkNodeTrackerTx = list.size() >=1?(SynkNodeTrackerTx) list.get(0): null; 

					top = SynkNodeTrackerTxDataHolder.convertToJSON(_SynkNodeTrackerTx, null, false, true, true);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                SynkNodeTrackerTx  _SynkNodeTrackerTx = SynkNodeTrackerTxDS.getInstance().getById(synchTrackingEntry.getObjectId());
                ...
                if (  _SynkNodeTrackerTx != null) {
    				totalCount++;
                    JSONObject json = SynkNodeTrackerTxDataHolder.convertToJSON(_SynkNodeTrackerTx, null, false, true, true);
    ...
    protected List prepareReturnData(HttpServletRequest request, SynkNodeTrackerTx target){
    ...
    protected List prepareReturnData(HttpServletRequest request, SynkNodeTrackerTx target, boolean isList){
        ...
        if (isList) {
            ...
            List list = SynkNodeTrackerTxDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                _SynkNodeTrackerTx = target;
            } else if (arg.equals("last")) {
                if (list.size() > 0)
                    _SynkNodeTrackerTx = (SynkNodeTrackerTx) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _SynkNodeTrackerTx = (SynkNodeTrackerTx) list.get(0);
            } else {
