...
import com.autosite.holder.CleanerGarmentTypeDataHolder;


public class CleanerGarmentTypeAjaxAction extends CleanerGarmentTypeAction {

    public CleanerGarmentTypeAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        CleanerGarmentType target = null;
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (CleanerGarmentType) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = CleanerGarmentTypeDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            CleanerGarmentType _CleanerGarmentType = CleanerGarmentTypeDS.getInstance().getById(id);
            if (_CleanerGarmentType == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _CleanerGarmentType);
                if (field != null)
            ...
            if ( _CleanerGarmentType == null) {
                ...
                String field = getFieldByName(fieldName, _CleanerGarmentType);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"cleanerGarmentType-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerGarmentType _CleanerGarmentType = (CleanerGarmentType) iterator.next();

                buf.append("<div id=\"cleanerGarmentType-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("title")) {
                    buf.append("<div id=\"cleanerGarmentType-ajax-title\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerGarmentType.getTitle()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("imagePath")) {
                    buf.append("<div id=\"cleanerGarmentType-ajax-imagePath\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerGarmentType.getImagePath()));
                    buf.append("</div>");
            ...
            List fieldsTitle = CleanerGarmentTypeDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerGarmentTypeDataHolder _CleanerGarmentType  = new CleanerGarmentTypeDataHolder( (CleanerGarmentType) iterator.next());
                gen.addTableRow(_CleanerGarmentType);
			}
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = CleanerGarmentTypeDataHolder.convertToJSON(_CleanerGarmentType, fieldSet, ignoreFieldSet, false);
...
*/
                ...
                CleanerGarmentType _CleanerGarmentType = list.size() >=1?(CleanerGarmentType) list.get(0): null; 

					top = CleanerGarmentTypeDataHolder.convertToJSON(_CleanerGarmentType, null, false, true);

/*
				if ( _CleanerGarmentType != null) {
	                top.put("type", "object");
		            top.put("id", ""+_CleanerGarmentType.getId());
		            JSONArray array = new JSONArray();
...
*/
                    if ( isSynchRequired() ){
		                try {
	                        if ( synchLedger == null)
			                    synchLedger = processSynchRequest(request, _CleanerGarmentType);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                CleanerGarmentType  _CleanerGarmentType = CleanerGarmentTypeDS.getInstance().getById(synchTrackingEntry.getObjectId());
                
//                JSONObject json = new JSONObject();
                JSONObject json = CleanerGarmentTypeDataHolder.convertToJSON(_CleanerGarmentType, null, false, true);
...
*/
            ...
            buf.append("sendFormAjax('/cleanerGarmentTypeAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/cleanerGarmentTypeAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_cleanerGarmentType(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayCleanerGarmentType\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_cleanerGarmentType(){\n";
            importedScripts +=     "xmlhttpPostXX('cleanerGarmentTypeFormAddDis','/cleanerGarmentTypeAction.html', 'resultDisplayCleanerGarmentType', '${ajax_response_fields}', responseCallback_cleanerGarmentType);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_cleanerGarmentType(){\n";
            importedScripts +=     "clearFormXX('cleanerGarmentTypeFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_cleanerGarmentType(){\n";
            importedScripts +=     "backToXX('cleanerGarmentTypeFormAddDis','resultDisplayCleanerGarmentType');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"cleanerGarmentTypeFormAddDis\" method=\"POST\" action=\"/cleanerGarmentTypeAction.html\" id=\"cleanerGarmentTypeFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_cleanerGarmentType()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_cleanerGarmentType()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayCleanerGarmentType\"></span>");
			buf.append("<a href=\"javascript:showform_cleanerGarmentType()\">Show form</a><br>");
	        buf.append("');");
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerGarmentType target){
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerGarmentType target, boolean isList){
        ...
        if (isList) {
            ...
            List list = CleanerGarmentTypeDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                _CleanerGarmentType = target;
            } else if (arg.equals("last")) {
                if (list.size() > 0)
                    _CleanerGarmentType = (CleanerGarmentType) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _CleanerGarmentType = (CleanerGarmentType) list.get(0);
            } else {
    ...
    private static Logger m_logger = Logger.getLogger( CleanerGarmentTypeAjaxAction.class);
}
