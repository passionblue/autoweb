...
import com.autosite.holder.CleanerUserDataHolder;


public class CleanerUserAjaxAction extends CleanerUserAction {

    public CleanerUserAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (CleanerUser) working.get("target");
            }
            catch (Exception e) {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            CleanerUser _CleanerUser = CleanerUserDS.getInstance().getById(id);
            if (_CleanerUser == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _CleanerUser);
                if (field != null)
            ...
            if ( _CleanerUser == null) {
                ...
                String field = getFieldByName(fieldName, _CleanerUser);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"cleanerUser-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerUser _CleanerUser = (CleanerUser) iterator.next();

                buf.append("<div id=\"cleanerUser-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("autositeUserId")) {
                    buf.append("<div id=\"cleanerUser-ajax-autositeUserId\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerUser.getAutositeUserId()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("inactivated")) {
                    buf.append("<div id=\"cleanerUser-ajax-inactivated\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerUser.getInactivated()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"cleanerUser-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerUser.getTimeCreated()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeUpdated")) {
                    buf.append("<div id=\"cleanerUser-ajax-timeUpdated\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerUser.getTimeUpdated()));
                    buf.append("</div>");
            ...
            List fieldsTitle = CleanerUserDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerUserDataHolder _CleanerUser  = new CleanerUserDataHolder( (CleanerUser) iterator.next());
                gen.addTableRow(_CleanerUser);
			}
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					// Fields
		            if ( ignoreFieldSet || fieldSet.contains("autositeUserId")) 
			            json.put("autositeUserId", ""+_CleanerUser.getAutositeUserId());
		            if ( ignoreFieldSet || fieldSet.contains("inactivated")) 
			            json.put("inactivated", ""+_CleanerUser.getInactivated());
		            array.put(json);
				}
                ...
                CleanerUser _CleanerUser = list.size() >=1?(CleanerUser) list.get(0): null; 

				if ( _CleanerUser != null) {
		            top.put("id", ""+_CleanerUser.getId());
		            JSONArray array = new JSONArray();

					// Fields
		            JSONObject jsonAutositeUserId = new JSONObject();
		            jsonAutositeUserId.put("name", "autositeUserId");
		            jsonAutositeUserId.put("value", ""+_CleanerUser.getAutositeUserId());
		            array.put(jsonAutositeUserId);
		            JSONObject jsonInactivated = new JSONObject();
		            jsonInactivated.put("name", "inactivated");
		            jsonInactivated.put("value", ""+_CleanerUser.getInactivated());
		            array.put(jsonInactivated);
            ...
            buf.append("sendFormAjax('/cleanerUserAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/cleanerUserAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            if ( ignoreFieldSet || fieldSet.contains("inactivated")) {
                String value = WebUtil.display(request.getParameter("inactivated"));

                if ( forceHiddenSet.contains("inactivated")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"inactivated\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Inactivated</div>");
            buf.append("<INPUT NAME=\"inactivated\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_cleanerUser(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayCleanerUser\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_cleanerUser(){\n";
            importedScripts +=     "xmlhttpPostXX('cleanerUserFormAddDis','/cleanerUserAction.html', 'resultDisplayCleanerUser', '${ajax_response_fields}', responseCallback_cleanerUser);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_cleanerUser(){\n";
            importedScripts +=     "clearFormXX('cleanerUserFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_cleanerUser(){\n";
            importedScripts +=     "backToXX('cleanerUserFormAddDis','resultDisplayCleanerUser');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"cleanerUserFormAddDis\" method=\"POST\" action=\"/cleanerUserAction.html\" id=\"cleanerUserFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Inactivated</div>");
        buf.append("<input class=\"field\" id=\"inactivated\" type=\"text\" size=\"70\" name=\"inactivated\" value=\"\"/> <span></span>");
		buf.append("<br/>");


			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_cleanerUser()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_cleanerUser()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayCleanerUser\"></span>");
			buf.append("<a href=\"javascript:showform_cleanerUser()\">Show form</a><br>");
	        buf.append("');");
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerUser target){
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerUser target, boolean isList){
        ...
        if (isList) {
            ...
            List list = CleanerUserDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                ...
                if (list.size() > 0)
                    _CleanerUser = (CleanerUser) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _CleanerUser = (CleanerUser) list.get(0);
            } else {
    ...
    private static Logger m_logger = Logger.getLogger( CleanerUserAjaxAction.class);
}
