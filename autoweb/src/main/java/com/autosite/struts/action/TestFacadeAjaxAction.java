...
public class TestFacadeAjaxAction extends TestFacadeAction {

    private static Logger m_logger = Logger.getLogger( TestFacadeAjaxAction.class);

    public TestFacadeAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        TestFacadeDataHolder target = null;
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (TestFacadeDataHolder) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = TestFacadeDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            if (_TestFacade == null) {
                ...
                String field = getField(request, response, _TestFacade);
                if (field != null)
            ...
            if ( _TestFacade == null) {
                ...
                String field = getFieldByName(fieldName, _TestFacade);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"testFacade-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                TestFacadeDataHolder _TestFacade = (TestFacadeDataHolder) iterator.next();

                buf.append("<div id=\"testFacade-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("data")) {
                    buf.append("<div id=\"testFacade-ajax-data\" "+itemClass+">");
                    buf.append(WebUtil.display(_TestFacade.getData()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"testFacade-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_TestFacade.getTimeCreated()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("facadeData")) {
                    buf.append("<div id=\"testFacade-ajax-facadeData\" "+itemClass+">");
                    buf.append(WebUtil.display(_TestFacade.getFacadeData()));
                    buf.append("</div>");
            ...
            List fieldsTitle = TestFacadeDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                TestFacadeDataHolder _TestFacade = (TestFacadeDataHolder) iterator.next();
                gen.addTableRow(_TestFacade);
			}
            ...
            buf.append("sendFormAjax('/testFacadeAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/testFacadeAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            if ( ignoreFieldSet || fieldSet.contains("facadeData")) {
                String value = WebUtil.display(request.getParameter("facadeData"));

                if ( forceHiddenSet.contains("facadeData")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"facadeData\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Facade Data</div>");
            buf.append("<INPUT NAME=\"facadeData\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_testFacade(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayTestFacade\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_testFacade(){\n";
            importedScripts +=     "xmlhttpPostXX('testFacadeFormAddDis','/testFacadeAction.html', 'resultDisplayTestFacade', '${ajax_response_fields}', responseCallback_testFacade);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_testFacade(){\n";
            importedScripts +=     "clearFormXX('testFacadeFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_testFacade(){\n";
            importedScripts +=     "backToXX('testFacadeFormAddDis','resultDisplayTestFacade');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"testFacadeFormAddDis\" method=\"POST\" action=\"/testFacadeAction.html\" id=\"testFacadeFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Facade Data</div>");
        buf.append("<input class=\"field\" id=\"facadeData\" type=\"text\" size=\"70\" name=\"facadeData\" value=\"\"/> <span></span>");
		buf.append("<br/>");


			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_testFacade()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_testFacade()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayTestFacade\"></span>");
			buf.append("<a href=\"javascript:showform_testFacade()\">Show form</a><br>");
	        buf.append("');");
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = TestFacadeDataHolder.convertToJSON(_TestFacade, fieldSet, ignoreFieldSet, true, true);

                    if ( isSynchRequired() ){
                        try {
                            synchLedger = findOrRegisterSynchLedger(request, _TestFacade.getId(), "${ios_scopePrefix}", true );
                            if ( synchLedger != null) {
                ...
                TestFacadeDataHolder _TestFacade = list.size() >=1?(TestFacadeDataHolder) list.get(0): null; 

					top = TestFacadeDataHolder.convertToJSON(_TestFacade, null, false, true, true);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                TestFacadeDataHolder  _TestFacade = TestFacadeDS.getInstance().getById(synchTrackingEntry.getObjectId());
                ...
                if (  _TestFacade != null) {
    				totalCount++;
                    JSONObject json = TestFacadeDataHolder.convertToJSON(_TestFacade, null, false, true, true);
    ...
    protected List prepareReturnData(HttpServletRequest request, TestFacadeDataHolder target){
    ...
    protected List prepareReturnData(HttpServletRequest request, TestFacadeDataHolder target, boolean isList){
        ...
        if (isList) {
            ...
            if (arg == null){
                ...
                if (list.size() > 0)
                    _TestFacade = (TestFacadeDataHolder) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _TestFacade = (TestFacadeDataHolder) list.get(0);
            } else {
