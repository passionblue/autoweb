...
public class ExpenseItemAjaxAction extends ExpenseItemAction {

    private static Logger m_logger = Logger.getLogger( ExpenseItemAjaxAction.class);

    public ExpenseItemAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (ExpenseItem) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = ExpenseItemDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            ExpenseItem _ExpenseItem = ExpenseItemDS.getInstance().getById(id);
            if (_ExpenseItem == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _ExpenseItem);
                if (field != null)
            ...
            if ( _ExpenseItem == null) {
                ...
                String field = getFieldByName(fieldName, _ExpenseItem);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"expenseItem-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                ExpenseItem _ExpenseItem = (ExpenseItem) iterator.next();

                buf.append("<div id=\"expenseItem-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("expenseCategoryId")) {
                    buf.append("<div id=\"expenseItem-ajax-expenseCategoryId\" "+itemClass+">");
                    buf.append(WebUtil.display(_ExpenseItem.getExpenseCategoryId()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("expenseItem")) {
                    buf.append("<div id=\"expenseItem-ajax-expenseItem\" "+itemClass+">");
                    buf.append(WebUtil.display(_ExpenseItem.getExpenseItem()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"expenseItem-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_ExpenseItem.getTimeCreated()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeUpdated")) {
                    buf.append("<div id=\"expenseItem-ajax-timeUpdated\" "+itemClass+">");
                    buf.append(WebUtil.display(_ExpenseItem.getTimeUpdated()));
                    buf.append("</div>");
            ...
            if ( ignoreFieldSet || fieldSet.contains("expenseCategoryId")) {
	            ...
	            buf.append("Expense Category Id");
	            buf.append("</td>");
			}
            if ( ignoreFieldSet || fieldSet.contains("expenseItem")) {
	            buf.append("<td "+ tdBoldStyleStr +" >");
            ...
            List fieldsTitle = ExpenseItemDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                ExpenseItemDataHolder _ExpenseItem  = new ExpenseItemDataHolder( (ExpenseItem) iterator.next());
                gen.addTableRow(_ExpenseItem);
			}
            ...
            buf.append("sendFormAjax('/expenseItemAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/expenseItemAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");


            if ( ignoreFieldSet || fieldSet.contains("expenseCategoryId")) {
                String value = WebUtil.display(request.getParameter("expenseCategoryId"));

                if ( forceHiddenSet.contains("expenseCategoryId")){
                    buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"expenseCategoryId\"  value=\""+value+"\">");
                } else {

			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Expense Category Id</div>");
            buf.append("<select id=\"requiredField\" name=\"expenseCategoryId\">");
            buf.append("<option value=\"\" >- Please Select -</option>");

            List dropMenus = DropMenuUtil.getDropMenus("ExpenseItemExpense Category IdOption");
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_expenseItem(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayExpenseItem\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_expenseItem(){\n";
            importedScripts +=     "xmlhttpPostXX('expenseItemFormAddDis','/expenseItemAction.html', 'resultDisplayExpenseItem', '${ajax_response_fields}', responseCallback_expenseItem);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_expenseItem(){\n";
            importedScripts +=     "clearFormXX('expenseItemFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_expenseItem(){\n";
            importedScripts +=     "backToXX('expenseItemFormAddDis','resultDisplayExpenseItem');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"expenseItemFormAddDis\" method=\"POST\" action=\"/expenseItemAction.html\" id=\"expenseItemFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");

		buf.append("<div class=\"ajaxFormLabel\"> Expense Category Id</div>");
        buf.append("<select class=\"field\" name=\"expenseCategoryId\" id=\"expenseCategoryId\">");
        buf.append("<option value=\"\" >- Please Select -</option>");
	...
	for(Iterator iter = _listExpenseCategory_expenseCategoryId.iterator(); iter.hasNext();){
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_expenseItem()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_expenseItem()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayExpenseItem\"></span>");
			buf.append("<a href=\"javascript:showform_expenseItem()\">Show form</a><br>");
	        buf.append("');");
...
//            DeviceSynchTracker tracker            = AutositeLedgerSynchTrackingManager.getInstance().getDeviceTracker(device);
            ...
            if (returnList || true) {
                ...
                if ( synchRequired ) {
                    ...
                    for (Iterator iterator = map.values().iterator(); iterator.hasNext();) {
                        ...
                        if ( WebUtil.isTrue(record.getDeleted())) {
                            ...
                            ExpenseItem _ExpenseItem = ExpenseItemDS.getInstance().getById(record.getRecordId());
                            ...
                            json                = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, fieldSet, ignoreFieldSet, true, true);
                            ...
                            JSONObject jsonTest    = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, fieldSet, ignoreFieldSet, false, true);
                    ...
                    for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                        ...
                        json     = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, fieldSet, ignoreFieldSet, true, true);
                        JSONObject json2    = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, fieldSet, ignoreFieldSet, false, true);
                ...
                ExpenseItem _ExpenseItem = list.size() >=1?(ExpenseItem) list.get(0): null; 

                    top = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, null, false, true, true);

                    if ( isSynchRequired() ){
                        try {
                            synchLedger = findOrRegisterSynchLedger(request, _ExpenseItem.getId(), "cleaner-pickup-delivery", true );
                            if ( synchLedger != null){
...
*/            
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                ExpenseItem  _ExpenseItem = ExpenseItemDS.getInstance().getById(synchTrackingEntry.getObjectId());
                ...
                if (  _ExpenseItem != null) {
    				totalCount++;
                    JSONObject json = ExpenseItemDataHolder.convertToJSON(_ExpenseItem, null, false, true, true);
    ...
    protected List prepareReturnData(HttpServletRequest request, ExpenseItem target){
    ...
    protected List prepareReturnData(HttpServletRequest request, ExpenseItem target, boolean isList){
        ...
        if (isList) {
            ...
            List list = ExpenseItemDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                ...
                if (list.size() > 0)
                    _ExpenseItem = (ExpenseItem) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _ExpenseItem = (ExpenseItem) list.get(0);
            } else {
