...
import com.autosite.holder.TestCoreDataHolder;


public class TestCoreAjaxAction extends TestCoreAction {

    private static Logger m_logger = Logger.getLogger( TestCoreAjaxAction.class);

    public TestCoreAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (TestCore) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = TestCoreDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            if (_TestCore == null) {
                ...
                String field = getField(request, response, _TestCore);
                if (field != null)
            ...
            if ( _TestCore == null) {
                ...
                String field = getFieldByName(fieldName, _TestCore);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"testCore-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                TestCore _TestCore = (TestCore) iterator.next();

                buf.append("<div id=\"testCore-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("data")) {
                    buf.append("<div id=\"testCore-ajax-data\" "+itemClass+">");
                    buf.append(WebUtil.display(_TestCore.getData()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("timeCreated")) {
                    buf.append("<div id=\"testCore-ajax-timeCreated\" "+itemClass+">");
                    buf.append(WebUtil.display(_TestCore.getTimeCreated()));
                    buf.append("</div>");
            ...
            List fieldsTitle = TestCoreDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                TestCoreDataHolder _TestCore  = new TestCoreDataHolder( (TestCore) iterator.next());
                gen.addTableRow(_TestCore);
			}
            ...
            buf.append("sendFormAjax('/testCoreAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/testCoreAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_testCore(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayTestCore\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_testCore(){\n";
            importedScripts +=     "xmlhttpPostXX('testCoreFormAddDis','/testCoreAction.html', 'resultDisplayTestCore', '${ajax_response_fields}', responseCallback_testCore);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_testCore(){\n";
            importedScripts +=     "clearFormXX('testCoreFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_testCore(){\n";
            importedScripts +=     "backToXX('testCoreFormAddDis','resultDisplayTestCore');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"testCoreFormAddDis\" method=\"POST\" action=\"/testCoreAction.html\" id=\"testCoreFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_testCore()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_testCore()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayTestCore\"></span>");
			buf.append("<a href=\"javascript:showform_testCore()\">Show form</a><br>");
	        buf.append("');");
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = TestCoreDataHolder.convertToJSON(_TestCore, fieldSet, ignoreFieldSet, true, true);

                    if ( isSynchRequired() ){
                        try {
                            synchLedger = findOrRegisterSynchLedger(request, _TestCore.getId(), "${ios_scopePrefix}", true );
                            if ( synchLedger != null) {
                ...
                TestCore _TestCore = list.size() >=1?(TestCore) list.get(0): null; 

					top = TestCoreDataHolder.convertToJSON(_TestCore, null, false, true, true);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                TestCore  _TestCore = TestCoreDS.getInstance().getById(synchTrackingEntry.getObjectId());
                ...
                if (  _TestCore != null) {
    				totalCount++;
                    JSONObject json = TestCoreDataHolder.convertToJSON(_TestCore, null, false, true, true);
    ...
    protected List prepareReturnData(HttpServletRequest request, TestCore target){
    ...
    protected List prepareReturnData(HttpServletRequest request, TestCore target, boolean isList){
        ...
        if (isList) {
            ...
            if (arg == null){
                ...
                if (list.size() > 0)
                    _TestCore = (TestCore) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _TestCore = (TestCore) list.get(0);
            } else {
