...
public class GenSimpleAjaxAction extends GenSimpleAction {

    public GenSimpleAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        GenSimpleDataHolder target = null;
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (GenSimpleDataHolder) working.get("target");
            }
            catch (Exception e) {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            if (_GenSimple == null) {
                ...
                String field = getField(request, response, _GenSimple);
                if (field != null)
            ...
            if ( _GenSimple == null) {
                ...
                String field = getFieldByName(fieldName, _GenSimple);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"genSimple-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                GenSimpleDataHolder _GenSimple = (GenSimpleDataHolder) iterator.next();

                buf.append("<div id=\"genSimple-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("data")) {
                    buf.append("<div id=\"genSimple-ajax-data\" "+itemClass+">");
                    buf.append(WebUtil.display(_GenSimple.getData()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("active")) {
                    buf.append("<div id=\"genSimple-ajax-active\" "+itemClass+">");
                    buf.append(WebUtil.display(_GenSimple.getActive()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("extString")) {
                    buf.append("<div id=\"genSimple-ajax-extString\" "+itemClass+">");
                    buf.append(WebUtil.display(_GenSimple.getExtString()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("extInt")) {
                    buf.append("<div id=\"genSimple-ajax-extInt\" "+itemClass+">");
                    buf.append(WebUtil.display(_GenSimple.getExtInt()));
                    buf.append("</div>");
            ...
            List fieldsTitle = GenSimpleDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                GenSimpleDataHolder _GenSimple = (GenSimpleDataHolder) iterator.next();
                gen.addTableRow(_GenSimple);
			}
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					// Fields
		            if ( ignoreFieldSet || fieldSet.contains("data")) 
			            json.put("data", ""+_GenSimple.getData());
		            if ( ignoreFieldSet || fieldSet.contains("active")) 
			            json.put("active", ""+_GenSimple.getActive());
		            if ( ignoreFieldSet || fieldSet.contains("extString")) 
			            json.put("extString", ""+_GenSimple.getExtString());
		            if ( ignoreFieldSet || fieldSet.contains("extInt")) 
			            json.put("extInt", ""+_GenSimple.getExtInt());
		            array.put(json);
				}
                ...
                GenSimpleDataHolder _GenSimple = list.size() >=1?(GenSimpleDataHolder) list.get(0): null; 

				if ( _GenSimple != null) {
		            top.put("id", ""+_GenSimple.getId());
		            JSONArray array = new JSONArray();

					// Fields
		            JSONObject jsonData = new JSONObject();
		            jsonData.put("name", "data");
		            jsonData.put("value", ""+_GenSimple.getData());
		            array.put(jsonData);
		            JSONObject jsonActive = new JSONObject();
		            jsonActive.put("name", "active");
		            jsonActive.put("value", ""+_GenSimple.getActive());
		            array.put(jsonActive);
		            JSONObject jsonExtString = new JSONObject();
		            jsonExtString.put("name", "extString");
		            jsonExtString.put("value", ""+_GenSimple.getExtString());
		            array.put(jsonExtString);
		            JSONObject jsonExtInt = new JSONObject();
		            jsonExtInt.put("name", "extInt");
		            jsonExtInt.put("value", ""+_GenSimple.getExtInt());
            ...
            buf.append("sendFormAjax('/genSimpleAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/genSimpleAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
			...
			buf.append("<div class=\"ajaxFormLabel\" style=\"font-weight:bold;\">Ext String</div>");
            buf.append("<INPUT NAME=\"extString\" type=\"text\" size=\"70\" value=\""+value+"\"></INPUT>");
			buf.append("<br/>");
			}
			}
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_genSimple(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayGenSimple\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_genSimple(){\n";
            importedScripts +=     "xmlhttpPostXX('genSimpleFormAddDis','/genSimpleAction.html', 'resultDisplayGenSimple', '${ajax_response_fields}', responseCallback_genSimple);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_genSimple(){\n";
            importedScripts +=     "clearFormXX('genSimpleFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_genSimple(){\n";
            importedScripts +=     "backToXX('genSimpleFormAddDis','resultDisplayGenSimple');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"genSimpleFormAddDis\" method=\"POST\" action=\"/genSimpleAction.html\" id=\"genSimpleFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
		buf.append("<div class=\"ajaxFormLabel\"> Ext String</div>");
        buf.append("<input class=\"field\" id=\"extString\" type=\"text\" size=\"70\" name=\"extString\" value=\"\"/> <span></span>");
		buf.append("<br/>");
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_genSimple()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_genSimple()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayGenSimple\"></span>");
			buf.append("<a href=\"javascript:showform_genSimple()\">Show form</a><br>");
	        buf.append("');");
    ...
    protected List prepareReturnData(HttpServletRequest request, GenSimpleDataHolder target){
    ...
    protected List prepareReturnData(HttpServletRequest request, GenSimpleDataHolder target, boolean isList){
        ...
        if (isList) {
            ...
            if (arg == null){
                ...
                if (list.size() > 0)
                    _GenSimple = (GenSimpleDataHolder) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _GenSimple = (GenSimpleDataHolder) list.get(0);
            } else {
    ...
    protected boolean isActionCmd(HttpServletRequest request){
        ...
        hasRequestValue(request, "register", "true")||
        super.isActionBasicCmd(request); 
    }

    private static Logger m_logger = Logger.getLogger( GenSimpleAjaxAction.class);
}
