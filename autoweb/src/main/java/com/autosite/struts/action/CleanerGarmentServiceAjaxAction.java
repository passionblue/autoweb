...
import com.autosite.holder.CleanerGarmentServiceDataHolder;


public class CleanerGarmentServiceAjaxAction extends CleanerGarmentServiceAction {

    public CleanerGarmentServiceAjaxAction(){
    }
    ...
    public Map exAjax(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        CleanerGarmentService target = null;
        ...
        if (isActionCmd(request)){
            ...
            try {
                ...
                target = (CleanerGarmentService) working.get("target");
                ...
                if (isSynchRequired()){
	                ...
	                if ( synchLedger != null) {
	                    target = CleanerGarmentServiceDS.getInstance().getById(synchLedger.getObjectId());
	                }
                } else {
        ...
        if (hasRequestValue(request, "ajaxOut", "getmodalstatus") ||
            ...
            CleanerGarmentService _CleanerGarmentService = CleanerGarmentServiceDS.getInstance().getById(id);
            if (_CleanerGarmentService == null) {
                ret.put("__error", "true");
                ret.put("__errorMsg", "Object Not Found");
                
            } else {
                String field = getField(request, response, _CleanerGarmentService);
                if (field != null)
            ...
            if ( _CleanerGarmentService == null) {
                ...
                String field = getFieldByName(fieldName, _CleanerGarmentService);
                if (field != null)
                    ret.put("__value", field);
                else 
                    ret.put("__value", "");
		...
		// classes 
            ...
            buf.append("<div id=\"cleanerGarmentService-ajax-frame\" "+frameClass+">");
            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerGarmentService _CleanerGarmentService = (CleanerGarmentService) iterator.next();

                buf.append("<div id=\"cleanerGarmentService-ajax-item\" "+listClass+">");

	            if ( ignoreFieldSet || fieldSet.contains("title")) {
                    buf.append("<div id=\"cleanerGarmentService-ajax-title\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerGarmentService.getTitle()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("note")) {
                    buf.append("<div id=\"cleanerGarmentService-ajax-note\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerGarmentService.getNote()));
                    buf.append("</div>");

				}
	            if ( ignoreFieldSet || fieldSet.contains("imagePath")) {
                    buf.append("<div id=\"cleanerGarmentService-ajax-imagePath\" "+itemClass+">");
                    buf.append(WebUtil.display(_CleanerGarmentService.getImagePath()));
                    buf.append("</div>");
            ...
            List fieldsTitle = CleanerGarmentServiceDataHolder.getFieldsName();

            gen.addTableRow(fieldsTitle, true);

            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
                CleanerGarmentServiceDataHolder _CleanerGarmentService  = new CleanerGarmentServiceDataHolder( (CleanerGarmentService) iterator.next());
                gen.addTableRow(_CleanerGarmentService);
			}
            ...
            if (returnList) {
	            ...
	            for (Iterator iterator = list.iterator(); iterator.hasNext();) {
					...
					JSONObject json = CleanerGarmentServiceDataHolder.convertToJSON(_CleanerGarmentService, fieldSet, ignoreFieldSet, false);
...
*/
                ...
                CleanerGarmentService _CleanerGarmentService = list.size() >=1?(CleanerGarmentService) list.get(0): null; 

					top = CleanerGarmentServiceDataHolder.convertToJSON(_CleanerGarmentService, null, false, true);

/*
				if ( _CleanerGarmentService != null) {
	                top.put("type", "object");
		            top.put("id", ""+_CleanerGarmentService.getId());
		            JSONArray array = new JSONArray();
...
*/
                    if ( isSynchRequired() ){
		                try {
	                        if ( synchLedger == null)
			                    synchLedger = processSynchRequest(request, _CleanerGarmentService);
            ...
            for (Iterator iterator = unsynched.iterator(); iterator.hasNext();) {
                ...
                CleanerGarmentService  _CleanerGarmentService = CleanerGarmentServiceDS.getInstance().getById(synchTrackingEntry.getObjectId());
                
//                JSONObject json = new JSONObject();
                JSONObject json = CleanerGarmentServiceDataHolder.convertToJSON(_CleanerGarmentService, null, false, true);
...
*/
            ...
            buf.append("sendFormAjax('/cleanerGarmentServiceAction.html','ajaxSubmitForm"+randNum+"', 'ajaxSubmit"+randNum+"', 'ajaxSubmitResult"+randNum+"')");
            buf.append("}");
            //buf.append("// -->");
            buf.append("</script>");

            buf.append("<form name=\"ajaxSubmitForm"+randNum+"\" method=\"POST\" action=\"/cleanerGarmentServiceAction.html\" id=\"ajaxSubmitForm"+randNum+"\" >");
            ...
            importedScripts += "\n";
            importedScripts += "function responseCallback_cleanerGarmentService(data){\n";
            importedScripts += "    document.getElementById(\"resultDisplayCleanerGarmentService\").innerHTML = data;\n";
            importedScripts += "}\n";
            importedScripts += "function sendRequest_cleanerGarmentService(){\n";
            importedScripts +=     "xmlhttpPostXX('cleanerGarmentServiceFormAddDis','/cleanerGarmentServiceAction.html', 'resultDisplayCleanerGarmentService', '${ajax_response_fields}', responseCallback_cleanerGarmentService);\n";
            importedScripts += "}\n";
            importedScripts += "function clearform_cleanerGarmentService(){\n";
            importedScripts +=     "clearFormXX('cleanerGarmentServiceFormAddDis');\n";
            importedScripts += "}\n";
            importedScripts += "function showform_cleanerGarmentService(){\n";
            importedScripts +=     "backToXX('cleanerGarmentServiceFormAddDis','resultDisplayCleanerGarmentService');\n";
            importedScripts += "}\n";
			...
			buf.append("<form name=\"cleanerGarmentServiceFormAddDis\" method=\"POST\" action=\"/cleanerGarmentServiceAction.html\" id=\"cleanerGarmentServiceFormAddDis\">");
			buf.append("<INPUT TYPE=\"HIDDEN\" NAME=\"add\" value=\"true\">");
		...
		buf.append("<br/>");
			...
			buf.append("<a id=\"formSubmit_ajax_simpleform\" href=\"javascript:sendRequest_cleanerGarmentService()\">Submit</a><br>");
            buf.append("<a href=\"javascript:clearform_cleanerGarmentService()\">Clear Form</a><br>");
		    //buf.append("<a href=\"/moveTo.html?dest=<%=cancelPage%>\">Cancel</a><br>");
			buf.append("</form>");
			buf.append("<span id=\"resultDisplayCleanerGarmentService\"></span>");
			buf.append("<a href=\"javascript:showform_cleanerGarmentService()\">Show form</a><br>");
	        buf.append("');");
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerGarmentService target){
    ...
    protected List prepareReturnData(HttpServletRequest request, CleanerGarmentService target, boolean isList){
        ...
        if (isList) {
            ...
            List list = CleanerGarmentServiceDS.getInstance().getBySiteId(site.getId());
            ...
            if (arg == null){
                _CleanerGarmentService = target;
            } else if (arg.equals("last")) {
                if (list.size() > 0)
                    _CleanerGarmentService = (CleanerGarmentService) list.get(list.size()-1);
            } else if (arg.equals("first")) {
                if (list.size() > 0)
                    _CleanerGarmentService = (CleanerGarmentService) list.get(0);
            } else {
            ...
            ret = new ArrayList();
            ret.add(_CleanerGarmentService);
    ...
    private static Logger m_logger = Logger.getLogger( CleanerGarmentServiceAjaxAction.class);
}
