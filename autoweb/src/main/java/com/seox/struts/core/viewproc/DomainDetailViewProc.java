...
public class DomainDetailViewProc implements ViewProc {
    
    private static Logger m_logger = Logger.getLogger(DomainDetailViewProc.class);
    public String getName() {
        return "DomainDetailViewProc";
    }
...
/*        
        if (!nocache && 
             session.getAttribute("kv_domain_detail") != null &&
             session.getAttribute("kv_domains") != null ) {
...
*/        
        ...
        String domainReq = (String) params.get("domain");
        
        m_logger.debug(params.get("domain"));
        
        m_logger.info("Domain " + domainReq + " is in the request for detail view");
        ...
        userBO.removeSessionSave("kv_domains");
        ...
        for(Iterator iter = userDomains.iterator(); iter.hasNext();) {
            ...
            if (dv.getDomain().equals(domainReq)) {
                session.setAttribute("kv_domain_detail", dv);
                userBO.setSessionSave("kv_domain_detail", dv);
            }
        }
