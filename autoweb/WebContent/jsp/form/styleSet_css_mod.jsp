...
<%
	//>>START<<
	List linkConfigs = LinkStyleConfigDS.getInstance().getBySiteId(site.getId());
	
	StyleConfig styleConfig = StyleConfigDS.getInstance().getById(_StyleSet.getStyleId());
	StyleConfig dataStyleConfig = StyleConfigDS.getInstance().getById(_StyleSet.getDataStyleId());
	LinkStyleConfig linkStyleConfig = LinkStyleConfigDS.getInstance().getById(_StyleSet.getLinkStyleId());
...
%>		
            <option value="<%=styleCfg.getId()%>" <%=HtmlUtil.getOptionSelect(_StyleSet.getStyleId(), styleCfg.getId())%> ><%=styleCfg.getStyleKey() + " (" + styleCfg.getId() + ")" %></option>
<%	} %>            

        </select>  

<%	if (styleConfig != null) {%>
		<a href="/v_style_config_edit.html?id=<%=styleConfig.getId() %>"> Edit</a>
<% 	} %>		
...
<%
	for(Iterator iter = styleConfigs.iterator(); iter.hasNext();){
		StyleConfig styleCfg = (StyleConfig) iter.next();
%>		
            <option value="<%=styleCfg.getId()%>" <%=HtmlUtil.getOptionSelect(_StyleSet.getDataStyleId(), styleCfg.getId())%> ><%=styleCfg.getStyleKey() + " (" + styleCfg.getId() + ")" %></option>
<%	} %>            
        </select>  
<%	if (dataStyleConfig != null) {%>
		<a href="/v_style_config_edit.html?id=<%=dataStyleConfig.getId() %>"> Edit</a>
<% 	} %>		
...
<%
	for(Iterator iter = linkConfigs.iterator(); iter.hasNext();){
		LinkStyleConfig styleCfg = (LinkStyleConfig) iter.next();
%>		
            <option value="<%=styleCfg.getId()%>" <%=HtmlUtil.getOptionSelect(_StyleSet.getLinkStyleId(), styleCfg.getId())%> ><%=styleCfg.getStyleKey() + " (" + styleCfg.getId() + ")" %></option>
<%	} %>            
        </select>  
<%	if (linkStyleConfig != null) {%>
		<a href="/v_link_style_config_edit.html?id=<%=linkStyleConfig.getId() %>"> Edit</a>
<% 	} %>		
        <span></span>
    </div>      
	</div><div class="clear"></div>
