...
<% 
    ...
    String _theme_idValue= WebUtil.display((String)reqParams.get("themeId"));
    String _css_indexValue= WebUtil.display((String)reqParams.get("cssIndex"));
    String _css_importValue= WebUtil.display((String)reqParams.get("cssImport"));
    String _layout_indexValue= WebUtil.display((String)reqParams.get("layoutIndex"));
...
<a href="/v_site_config_style_home.html"> SiteConfigStyle Home </a>
<%
	
	List list = null;
	list = SiteConfigStyleDS.getInstance().getBySiteId(site.getId());

%>

<div id="siteConfigStyleList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		SiteConfigStyle _SiteConfigStyle = (SiteConfigStyle) iter.next();	
%>

	<div id="siteConfigStyleFrame<%=_SiteConfigStyle.getId() %>" >

		<div id="themeId" >
			themeId:<%= _SiteConfigStyle.getThemeId() %>
		</div>
		<div id="cssIndex" >
			cssIndex:<%= _SiteConfigStyle.getCssIndex() %>
		</div>
		<div id="cssImport" >
			cssImport:<%= _SiteConfigStyle.getCssImport() %>
		</div>
		<div id="layoutIndex" >
			layoutIndex:<%= _SiteConfigStyle.getLayoutIndex() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _SiteConfigStyle.getTimeCreated() %>
		</div>
		<div id="timeUpdated" >
			timeUpdated:<%= _SiteConfigStyle.getTimeUpdated() %>
		</div>
		<div>
		<a id="siteConfigStyleDeleteButton" href="javascript:deleteThis('/siteConfigStyleAction.html',<%= _SiteConfigStyle.getId()%>,'siteConfigStyleFrame<%=_SiteConfigStyle.getId()%>' );" > 
...
<form name="siteConfigStyleFormAdd" method="POST" action="/siteConfigStyleAction.html" id="siteConfigStyleFormAdd">
    ...
    <div id="siteConfigStyleForm_themeId_dropdown" class="formFieldDropDown" >       
        <select class="field" name="themeId" id="_ffd_themeId">
...
<%	} %>            
    ...
    <div id="siteConfigStyleForm_cssImport_text" class="formFieldText" >       
        <input class="field" id="_ffd_cssImport" type="text" size="70" name="cssImport" value="<%=WebUtil.display(_css_importValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
    ...
    <div id="siteConfigStyleForm_layoutIndex_text" class="formFieldText" >       
        <input class="field" id="_ffd_layoutIndex" type="text" size="70" name="layoutIndex" value="<%=WebUtil.display(_layout_indexValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="siteConfigStyleFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/siteConfigStyleAction.html', 'siteConfigStyleFormAdd', 'formSubmit_ajax', 'siteConfigStyle');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplaySiteConfigStyle").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('siteConfigStyleFormAddDis','/siteConfigStyleAction.html', 'resultDisplaySiteConfigStyle', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="siteConfigStyleFormAddDis" method="POST" action="/siteConfigStyleAction.html" id="siteConfigStyleFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Theme Id</div>
        <select class="field" name="themeId" id="themeId">
        <option value="" >- Please Select -</option>
<%
	_listThemeAggregator_themeId = ThemeAggregatorDS.getInstance().getBySiteId(site.getId());
	for(Iterator iter = _listThemeAggregator_themeId.iterator(); iter.hasNext();){
		ThemeAggregator _obj = (ThemeAggregator) iter.next();
%>		
            <option value="<%=_obj.getId()%>" <%=HtmlUtil.getOptionSelect(_theme_idValue, String.valueOf(_obj.getId()))%> ><%=_obj.getThemeName() + " (" + _obj.getId() + ")" %></option>
<%	} %>            
		...
		<br/>
		 <div class="ajaxFormLabel"> Css Import</div>
        <input class="field" id="cssImport" type="text" size="70" name="cssImport" value="<%=WebUtil.display(_css_importValue)%>"/> <span></span>
		<br/>
		 <div class="ajaxFormLabel"> Layout Index</div>
        <input class="field" id="layoutIndex" type="text" size="70" name="layoutIndex" value="<%=WebUtil.display(_layout_indexValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('siteConfigStyleFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplaySiteConfigStyle"></span>
<a href="javascript:backToXX('siteConfigStyleFormAddDis','resultDisplaySiteConfigStyle')">show back</a><br>
<hr/>
