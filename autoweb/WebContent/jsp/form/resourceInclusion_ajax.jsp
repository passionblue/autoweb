...
<% 
    ...
    String _includeValue= WebUtil.display((String)reqParams.get("include"));
    String _resource_typeValue= WebUtil.display((String)reqParams.get("resourceType"));
%> 

<a href="/v_resource_inclusion_home.html"> ResourceInclusion Home </a>
<%
	
	List list = null;
	list = ResourceInclusionDS.getInstance().getBySiteId(site.getId());

%>

<div id="resourceInclusionList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ResourceInclusion _ResourceInclusion = (ResourceInclusion) iter.next();	
%>

	<div id="resourceInclusionFrame<%=_ResourceInclusion.getId() %>" >

		<div id="name" >
			name:<%= _ResourceInclusion.getName() %>
		</div>
		<div id="include" >
			include:<%= _ResourceInclusion.getInclude() %>
		</div>
		<div id="resourceType" >
			resourceType:<%= _ResourceInclusion.getResourceType() %>
		</div>
		<div>
		<a id="resourceInclusionDeleteButton" href="javascript:deleteThis('/resourceInclusionAction.html',<%= _ResourceInclusion.getId()%>,'resourceInclusionFrame<%=_ResourceInclusion.getId()%>' );" > 
...
<form name="resourceInclusionFormAdd" method="POST" action="/resourceInclusionAction.html" id="resourceInclusionFormAdd">
    ...
    <div id="resourceInclusionForm_name_text" class="formFieldText" >       
        <input class="field" id="_ffd_name" type="text" size="70" name="name" value="<%=WebUtil.display(_nameValue)%>"/> <span></span>
    </div>      
    ...
    <div id="resourceInclusionForm_include_dropdown" class="formFieldDropDown" >       
        <select name="include" id="_ffd_include">
    ...
    <div id="resourceInclusionForm_resourceType_dropdown" class="formFieldDropDown" >       
        <select class="field" name="resourceType" id="_ffd_resourceType">
        <option value="" >- Please Select -</option>
<%
	List dropMenusResourceType = DropMenuUtil.getDropMenus("ResourceInclusionResourceTypeOption");
...
<%} %>
        ...
        <div id="resourceInclusionFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/resourceInclusionAction.html', 'resourceInclusionFormAdd', 'formSubmit_ajax', 'resourceInclusion');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayResourceInclusion").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('resourceInclusionFormAddDis','/resourceInclusionAction.html', 'resultDisplayResourceInclusion', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="resourceInclusionFormAddDis" method="POST" action="/resourceInclusionAction.html" id="resourceInclusionFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Name</div>
        <input class="field" id="name" type="text" size="70" name="name" value="<%=WebUtil.display(_nameValue)%>"/> <span></span>
		<br/>
		 <div class="ajaxFormLabel"> Include</div>
        <select name="include" id="include">
        <option value="0" <%=HtmlUtil.getOptionSelect("0", _includeValue)%>>No</option>
        <option value="1" <%=HtmlUtil.getOptionSelect("1", _includeValue)%>>Yes</option>
        </select><span></span>
		<br/>
		 <div class="ajaxFormLabel"> Resource Type</div>
        <select class="field" name="resourceType" id="resourceType">
        <option value="" >- Please Select -</option>
<%
	dropMenusResourceType = DropMenuUtil.getDropMenus("ResourceInclusionType");
	for(Iterator iterItems = dropMenusResourceType.iterator(); iterItems.hasNext();){
		DropMenuItem it = (DropMenuItem) iterItems.next();
%>				
		<option value="<%=it.getValue() %>" <%=HtmlUtil.getOptionSelect(it.getValue(), _resource_typeValue)%>><%=it.getDisplay() %></option>
<%} %>
	...
	<a href="javascript:clearFormXX('resourceInclusionFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayResourceInclusion"></span>
<a href="javascript:backToXX('resourceInclusionFormAddDis','resultDisplayResourceInclusion')">show back</a><br>
<hr/>
