...
<% 
    ...
    String _agreeValue= WebUtil.display((String)reqParams.get("agree"));
%> 

<a href="/v_gen_flow_confirm_home.html"> GenFlowConfirm Home </a>
<%
	
	List list = null;
	list = GenFlowConfirmDS.getInstance().getBySiteId(site.getId());

%>

<div id="genFlowConfirmList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		GenFlowConfirmDataHolder _GenFlowConfirm = (GenFlowConfirmDataHolder) iter.next();	
%>

	<div id="genFlowConfirmFrame<%=_GenFlowConfirm.getId() %>" >

		<div id="agree" >
			agree:<%= _GenFlowConfirm.getAgree() %>
		</div>
		<div>
		<a id="genFlowConfirmDeleteButton" href="javascript:deleteThis('/genFlowConfirmAction.html',<%= _GenFlowConfirm.getId()%>,'genFlowConfirmFrame<%=_GenFlowConfirm.getId()%>' );" > 
...
<form name="genFlowConfirmFormAdd" method="POST" action="/genFlowConfirmAction.html" id="genFlowConfirmFormAdd">
    ...
    <div id="genFlowConfirmForm_agree_text" class="formFieldText" >       
        <input class="field" id="_ffd_agree" type="text" size="70" name="agree" value="<%=WebUtil.display(_agreeValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="genFlowConfirmFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/genFlowConfirmAction.html', 'genFlowConfirmFormAdd', 'formSubmit_ajax', 'genFlowConfirm');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayGenFlowConfirm").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('genFlowConfirmFormAddDis','/genFlowConfirmAction.html', 'resultDisplayGenFlowConfirm', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="genFlowConfirmFormAddDis" method="POST" action="/genFlowConfirmAction.html" id="genFlowConfirmFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Agree</div>
        <input class="field" id="agree" type="text" size="70" name="agree" value="<%=WebUtil.display(_agreeValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('genFlowConfirmFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayGenFlowConfirm"></span>
<a href="javascript:backToXX('genFlowConfirmFormAddDis','resultDisplayGenFlowConfirm')">show back</a><br>
<hr/>
