...
<script type="text/javascript" src="/genFlowStartAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_gen_flow_start_form.html?prv_returnPage=gen_flow_start_home"> Add New </a> |
            <a href="v_gen_flow_start_list.html?"> List Page </a> |
            <a href="v_gen_flow_start_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/genFlowStartAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/genFlowStartAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/genFlowStartAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/genFlowStartAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/genFlowStartAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/genFlowStartAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/genFlowStartAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
		</TD>        
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="genFlowStartForm_yourName_label" style="font-size: normal normal bold 10px verdana;" >Your Name </div>
    </td> 
    <td> 
	    <div id="genFlowStartForm_email_label" style="font-size: normal normal bold 10px verdana;" >Email </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<%
    ...
    for(Iterator iter = list.iterator();iter.hasNext();) {
        GenFlowStartDataHolder _GenFlowStart = (GenFlowStartDataHolder) iter.next();
		//TODO 
        fieldString += "\"" +  _GenFlowStart.getId() + "\",";
%>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> <%= _GenFlowStart.getId() %> </td>


    <td> 
	<form name="genFlowStartFormEditField_YourName_<%=_GenFlowStart.getId()%>" method="get" action="/genFlowStartAction.html" >


		<div id="genFlowStartForm_yourName_field">
	    <div id="genFlowStartForm_yourName_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="yourName" value="<%=WebUtil.display(_GenFlowStart.getYourName())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="yourName_<%= _GenFlowStart.getId()%>"><%=_GenFlowStart.getYourName() %></div>
            <a id="formSubmit" href="javascript:document.genFlowStartFormEditField_YourName_<%=_GenFlowStart.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_GenFlowStart.getId()%>', 'yourName', '<%=_GenFlowStart.getYourName()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="yourName">	Update </a>
			<a id="update_field_by_ajax_open_reply" href="#" rel="yourName">	Update </a>

	<INPUT TYPE="HIDDEN" NAME="editfield" value="true">
	<INPUT TYPE="HIDDEN" NAME="id" value="<%=_GenFlowStart.getId()%>">
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="gen_flow_start_home">
	</form>
    ...
    <td> 
	<form name="genFlowStartFormEditField_Email_<%=_GenFlowStart.getId()%>" method="get" action="/genFlowStartAction.html" >


		<div id="genFlowStartForm_email_field">
	    <div id="genFlowStartForm_email_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="email" value="<%=WebUtil.display(_GenFlowStart.getEmail())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="email_<%= _GenFlowStart.getId()%>"><%=_GenFlowStart.getEmail() %></div>
            <a id="formSubmit" href="javascript:document.genFlowStartFormEditField_Email_<%=_GenFlowStart.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_GenFlowStart.getId()%>', 'email', '<%=_GenFlowStart.getEmail()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
    ...
    <td>
    <form name="genFlowStartForm<%=_GenFlowStart.getId()%>2" method="get" action="/v_gen_flow_start_form.html" >
        <a href="javascript:document.genFlowStartForm<%=_GenFlowStart.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _GenFlowStart.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="gen_flow_start_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_gen_flow_start_form('<%=_GenFlowStart.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/genFlowStartAction.html?del=true&id=<%=_GenFlowStart.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_gen_flow_start('<%=_GenFlowStart.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_gen_flow_start_form(target){
		location.href='/v_gen_flow_start_form.html?cmd=edit&prv_returnPage=gen_flow_start_home&id=' + target;
	}

	function confirm_gen_flow_start(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_gen_flow_start(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/genFlowStartAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/genFlowStartAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
