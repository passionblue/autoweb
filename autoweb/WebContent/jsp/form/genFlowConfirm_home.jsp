...
<script type="text/javascript" src="/genFlowConfirmAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_gen_flow_confirm_form.html?prv_returnPage=gen_flow_confirm_home"> Add New </a> |
            <a href="v_gen_flow_confirm_list.html?"> List Page </a> |
            <a href="v_gen_flow_confirm_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/genFlowConfirmAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/genFlowConfirmAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
		</TD>        
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="genFlowConfirmForm_agree_label" style="font-size: normal normal bold 10px verdana;" >Agree </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<%
    ...
    for(Iterator iter = list.iterator();iter.hasNext();) {
        GenFlowConfirmDataHolder _GenFlowConfirm = (GenFlowConfirmDataHolder) iter.next();
		//TODO 
        fieldString += "\"" +  _GenFlowConfirm.getId() + "\",";
%>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> <%= _GenFlowConfirm.getId() %> </td>


    <td> 
	<form name="genFlowConfirmFormEditField_Agree_<%=_GenFlowConfirm.getId()%>" method="get" action="/genFlowConfirmAction.html" >


		<div id="genFlowConfirmForm_agree_field">
	    <div id="genFlowConfirmForm_agree_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="agree" value="<%=WebUtil.display(_GenFlowConfirm.getAgree())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="agree_<%= _GenFlowConfirm.getId()%>"><%=_GenFlowConfirm.getAgree() %></div>
            <a id="formSubmit" href="javascript:document.genFlowConfirmFormEditField_Agree_<%=_GenFlowConfirm.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_GenFlowConfirm.getId()%>', 'agree', '<%=_GenFlowConfirm.getAgree()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="agree">	Update </a>
			<a id="update_field_by_ajax_open_reply" href="#" rel="agree">	Update </a>

	<INPUT TYPE="HIDDEN" NAME="editfield" value="true">
	<INPUT TYPE="HIDDEN" NAME="id" value="<%=_GenFlowConfirm.getId()%>">
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="gen_flow_confirm_home">
	</form>
    ...
    <td>
    <form name="genFlowConfirmForm<%=_GenFlowConfirm.getId()%>2" method="get" action="/v_gen_flow_confirm_form.html" >
        <a href="javascript:document.genFlowConfirmForm<%=_GenFlowConfirm.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _GenFlowConfirm.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="gen_flow_confirm_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_gen_flow_confirm_form('<%=_GenFlowConfirm.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/genFlowConfirmAction.html?del=true&id=<%=_GenFlowConfirm.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_gen_flow_confirm('<%=_GenFlowConfirm.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_gen_flow_confirm_form(target){
		location.href='/v_gen_flow_confirm_form.html?cmd=edit&prv_returnPage=gen_flow_confirm_home&id=' + target;
	}

	function confirm_gen_flow_confirm(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_gen_flow_confirm(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/genFlowConfirmAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/genFlowConfirmAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
