...
<script type="text/javascript" src="/genFlowFinalAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_gen_flow_final_form.html?prv_returnPage=gen_flow_final_home"> Add New </a> |
            <a href="v_gen_flow_final_list.html?"> List Page </a> |
            <a href="v_gen_flow_final_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/genFlowFinalAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/genFlowFinalAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/genFlowFinalAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/genFlowFinalAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/genFlowFinalAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/genFlowFinalAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/genFlowFinalAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
		</TD>        
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="genFlowFinalForm_accept_label" style="font-size: normal normal bold 10px verdana;" >Accept </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<%
    ...
    for(Iterator iter = list.iterator();iter.hasNext();) {
        GenFlowFinalDataHolder _GenFlowFinal = (GenFlowFinalDataHolder) iter.next();
		//TODO 
        fieldString += "\"" +  _GenFlowFinal.getId() + "\",";
%>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> <%= _GenFlowFinal.getId() %> </td>


    <td> 
	<form name="genFlowFinalFormEditField_Accept_<%=_GenFlowFinal.getId()%>" method="get" action="/genFlowFinalAction.html" >


		<div id="genFlowFinalForm_accept_field">
	    <div id="genFlowFinalForm_accept_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="accept" value="<%=WebUtil.display(_GenFlowFinal.getAccept())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="accept_<%= _GenFlowFinal.getId()%>"><%=_GenFlowFinal.getAccept() %></div>
            <a id="formSubmit" href="javascript:document.genFlowFinalFormEditField_Accept_<%=_GenFlowFinal.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_GenFlowFinal.getId()%>', 'accept', '<%=_GenFlowFinal.getAccept()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="accept">	Update </a>
			<a id="update_field_by_ajax_open_reply" href="#" rel="accept">	Update </a>

	<INPUT TYPE="HIDDEN" NAME="editfield" value="true">
	<INPUT TYPE="HIDDEN" NAME="id" value="<%=_GenFlowFinal.getId()%>">
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="gen_flow_final_home">
	</form>
    ...
    <td>
    <form name="genFlowFinalForm<%=_GenFlowFinal.getId()%>2" method="get" action="/v_gen_flow_final_form.html" >
        <a href="javascript:document.genFlowFinalForm<%=_GenFlowFinal.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _GenFlowFinal.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="gen_flow_final_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_gen_flow_final_form('<%=_GenFlowFinal.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/genFlowFinalAction.html?del=true&id=<%=_GenFlowFinal.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_gen_flow_final('<%=_GenFlowFinal.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_gen_flow_final_form(target){
		location.href='/v_gen_flow_final_form.html?cmd=edit&prv_returnPage=gen_flow_final_home&id=' + target;
	}

	function confirm_gen_flow_final(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_gen_flow_final(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/genFlowFinalAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/genFlowFinalAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
