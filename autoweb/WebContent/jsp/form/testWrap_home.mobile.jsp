...
<script type="text/javascript" src="/testWrapAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_test_wrap_form.html?prv_returnPage=test_wrap_home"> Add New </a> |
            <a href="v_test_wrap_list.html?"> List Page </a> |
            <a href="v_test_wrap_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/testWrapAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/testWrapAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/testWrapAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/testWrapAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/testWrapAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/testWrapAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/testWrapAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="testWrapForm_data_label" style="font-size: normal normal bold 10px verdana;" >Data </div>
    </td> 
    <td> 
	    <div id="testWrapForm_wrapData_label" style="font-size: normal normal bold 10px verdana;" >Wrap Data </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	<form name="testWrapFormEditField_Data_<%=_TestWrap.getId()%>" method="get" action="/testWrapAction.html" >


		<div id="testWrapForm_data_field">
	    <div id="testWrapForm_data_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="data" value="<%=WebUtil.display(_TestWrap.getData())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="data_<%= _TestWrap.getId()%>"><%=_TestWrap.getData() %></div>
            <a id="formSubmit" href="javascript:document.testWrapFormEditField_Data_<%=_TestWrap.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_TestWrap.getId()%>', 'data', '<%=_TestWrap.getData()%>');">
	    	EditPrompt
	    	</a>
	...
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="test_wrap_home">
	</form>
    ...
    <td> 
	<form name="testWrapFormEditField_WrapData_<%=_TestWrap.getId()%>" method="get" action="/testWrapAction.html" >


		<div id="testWrapForm_wrapData_field">
	    <div id="testWrapForm_wrapData_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="wrapData" value="<%=WebUtil.display(_TestWrap.getWrapData())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="wrapData_<%= _TestWrap.getId()%>"><%=_TestWrap.getWrapData() %></div>
            <a id="formSubmit" href="javascript:document.testWrapFormEditField_WrapData_<%=_TestWrap.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_TestWrap.getId()%>', 'wrapData', '<%=_TestWrap.getWrapData()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="wrapData">	Update </a><br>
			<a id="update_field_by_ajax_open_reply" href="#" rel="wrapData">	Update </a><br>
			<a id="update_field_by_ajax_get2field" href="#" rel="wrapData">	Update Test With Get2Field value </a><br>
    ...
    <td>
    <form name="testWrapForm<%=_TestWrap.getId()%>2" method="get" action="/v_test_wrap_form.html" >
        <a href="javascript:document.testWrapForm<%=_TestWrap.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _TestWrap.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="test_wrap_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_test_wrap_form('<%=_TestWrap.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/testWrapAction.html?del=true&id=<%=_TestWrap.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_test_wrap('<%=_TestWrap.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_test_wrap_form(target){
		location.href='/v_test_wrap_form.html?cmd=edit&prv_returnPage=test_wrap_home&id=' + target;
	}

	function confirm_test_wrap(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_test_wrap(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/testWrapAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/testWrapAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
	...
	$(document).ready(function(){
		...
		$("a#update_field_by_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/testWrapAction.html?editfield=true&ajxr=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_open_reply").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testWrapAction.html?editfield=true&ajaxRequest=true&ajaxOut=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_get2field").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testWrapAction.html?editfield=true&ajxr=get2field&id="+_id+"&"+_fieldName+"="+ _val,
				beforeSend: function(jqXHR, settings){
					//alert("sending");
				},
				complete: function(jqXHR, textStatus){
		...
		$("a#partition_test_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/partitionTo.html?fmt=getpart&partType=custom&partId=partition-test-wrap",
		  		success: function(msg){ 
		     		alert(msg);
