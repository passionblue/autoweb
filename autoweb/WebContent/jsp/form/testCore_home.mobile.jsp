...
<script type="text/javascript" src="/testCoreAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_test_core_form.html?prv_returnPage=test_core_home"> Add New </a> |
            <a href="v_test_core_list.html?"> List Page </a> |
            <a href="v_test_core_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/testCoreAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/testCoreAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/testCoreAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/testCoreAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/testCoreAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/testCoreAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/testCoreAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="testCoreForm_data_label" style="font-size: normal normal bold 10px verdana;" >Data </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	<form name="testCoreFormEditField_Data_<%=_TestCore.getId()%>" method="get" action="/testCoreAction.html" >


		<div id="testCoreForm_data_field">
	    <div id="testCoreForm_data_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="data" value="<%=WebUtil.display(_TestCore.getData())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="data_<%= _TestCore.getId()%>"><%=_TestCore.getData() %></div>
            <a id="formSubmit" href="javascript:document.testCoreFormEditField_Data_<%=_TestCore.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_TestCore.getId()%>', 'data', '<%=_TestCore.getData()%>');">
	    	EditPrompt
	    	</a>
	...
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="test_core_home">
	</form>
    ...
    <td>
    <form name="testCoreForm<%=_TestCore.getId()%>2" method="get" action="/v_test_core_form.html" >
        <a href="javascript:document.testCoreForm<%=_TestCore.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _TestCore.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="test_core_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_test_core_form('<%=_TestCore.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/testCoreAction.html?del=true&id=<%=_TestCore.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_test_core('<%=_TestCore.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_test_core_form(target){
		location.href='/v_test_core_form.html?cmd=edit&prv_returnPage=test_core_home&id=' + target;
	}

	function confirm_test_core(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_test_core(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/testCoreAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/testCoreAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
	...
	$(document).ready(function(){
		...
		$("a#update_field_by_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/testCoreAction.html?editfield=true&ajxr=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_open_reply").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testCoreAction.html?editfield=true&ajaxRequest=true&ajaxOut=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_get2field").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testCoreAction.html?editfield=true&ajxr=get2field&id="+_id+"&"+_fieldName+"="+ _val,
				beforeSend: function(jqXHR, settings){
					//alert("sending");
				},
				complete: function(jqXHR, textStatus){
		...
		$("a#partition_test_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/partitionTo.html?fmt=getpart&partType=custom&partId=partition-test-core",
		  		success: function(msg){ 
		     		alert(msg);
