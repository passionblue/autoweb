...
<% 
    ...
    String _wrap_dataValue= WebUtil.display((String)reqParams.get("wrapData"));
%> 

<a href="/v_test_wrap_home.html"> TestWrap Home </a>
<%
	
	List list = null;
	list = TestWrapDS.getInstance().getBySiteId(site.getId());
...
%>
	...
	<div id="testCoreFrame<%=_TestWrap.getId() %>" >

		<div id="data" >
			data:<%= _TestWrap.getData() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _TestWrap.getTimeCreated() %>
		</div>
		<div id="wrapData" >
			wrapData:<%= _TestWrap.getWrapData() %>
		</div>
		<div>
		<a id="testCoreDeleteButton" href="javascript:deleteThis('/testWrapAction.html',<%= _TestWrap.getId()%>,'testCoreFrame<%=_TestWrap.getId()%>' );" > 
...
<form name="testWrapFormAdd" method="POST" action="/testWrapAction.html" id="testWrapFormAdd">
    ...
    <div id="testWrapForm_wrapData_text" class="formFieldText" >       
        <input class="field" id="_ffd_wrapData" type="text" size="70" name="wrapData" value="<%=WebUtil.display(_wrap_dataValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="testCoreFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/testCoreAction.html', 'testCoreFormAdd', 'formSubmit_ajax', 'testCore');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayTestWrap").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('testCoreFormAddDis','/testCoreAction.html', 'resultDisplayTestWrap', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="testCoreFormAddDis" method="POST" action="/testCoreAction.html" id="testCoreFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Wrap Data</div>
        <input class="field" id="wrapData" type="text" size="70" name="wrapData" value="<%=WebUtil.display(_wrap_dataValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('testCoreFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayTestWrap"></span>
<a href="javascript:backToXX('testCoreFormAddDis','resultDisplayTestWrap')">show back</a><br>
<hr/>
