...
<% 
    ...
    String _facade_dataValue= WebUtil.display((String)reqParams.get("facadeData"));
%> 

<a href="/v_test_facade_home.html"> TestFacade Home </a>
<%
	
	List list = null;
	list = TestFacadeDS.getInstance().getBySiteId(site.getId());
...
%>
	...
	<div id="testCoreFrame<%=_TestFacade.getId() %>" >

		<div id="data" >
			data:<%= _TestFacade.getData() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _TestFacade.getTimeCreated() %>
		</div>
		<div id="facadeData" >
			facadeData:<%= _TestFacade.getFacadeData() %>
		</div>
		<div>
		<a id="testCoreDeleteButton" href="javascript:deleteThis('/testFacadeAction.html',<%= _TestFacade.getId()%>,'testCoreFrame<%=_TestFacade.getId()%>' );" > 
...
<form name="testFacadeFormAdd" method="POST" action="/testFacadeAction.html" id="testFacadeFormAdd">
    ...
    <div id="testFacadeForm_facadeData_text" class="formFieldText" >       
        <input class="field" id="_ffd_facadeData" type="text" size="70" name="facadeData" value="<%=WebUtil.display(_facade_dataValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
...
function responseCallback(data){
	document.getElementById("resultDisplayTestFacade").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('testCoreFormAddDis','/testCoreAction.html', 'resultDisplayTestFacade', '${ajax_response_fields}', responseCallback);
}
</script>
...
<form name="testCoreFormAddDis" method="POST" action="/testCoreAction.html" id="testCoreFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Facade Data</div>
        <input class="field" id="facadeData" type="text" size="70" name="facadeData" value="<%=WebUtil.display(_facade_dataValue)%>"/> <span></span>
		<br/>
...
<span id="resultDisplayTestFacade"></span>
<a href="javascript:backToXX('testCoreFormAddDis','resultDisplayTestFacade')">show back</a><br>
<hr/>
