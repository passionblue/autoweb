...
<% 
    ...
    String _device_typeValue= WebUtil.display((String)reqParams.get("deviceType"));
    String _time_createdValue= WebUtil.display((String)reqParams.get("timeCreated"));
%> 

<a href="/v_autosite_remote_device_home.html"> AutositeRemoteDevice Home </a>
<%
	
	List list = null;
	list = AutositeRemoteDeviceDS.getInstance().getBySiteId(site.getId());

%>

<div id="autositeRemoteDeviceList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		AutositeRemoteDevice _AutositeRemoteDevice = (AutositeRemoteDevice) iter.next();	
%>

	<div id="autositeRemoteDeviceFrame<%=_AutositeRemoteDevice.getId() %>" >

		<div id="deviceId" >
			deviceId:<%= _AutositeRemoteDevice.getDeviceId() %>
		</div>
		<div id="deviceType" >
			deviceType:<%= _AutositeRemoteDevice.getDeviceType() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _AutositeRemoteDevice.getTimeCreated() %>
		</div>
		<div>
		<a id="autositeRemoteDeviceDeleteButton" href="javascript:deleteThis('/autositeRemoteDeviceAction.html',<%= _AutositeRemoteDevice.getId()%>,'autositeRemoteDeviceFrame<%=_AutositeRemoteDevice.getId()%>' );" > 
...
<form name="autositeRemoteDeviceFormAdd" method="POST" action="/autositeRemoteDeviceAction.html" id="autositeRemoteDeviceFormAdd">
    ...
    <div id="autositeRemoteDeviceForm_deviceType_text" class="formFieldText" >       
        <input class="field" id="_ffd_deviceType" type="text" size="70" name="deviceType" value="<%=WebUtil.display(_device_typeValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="autositeRemoteDeviceFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/autositeRemoteDeviceAction.html', 'autositeRemoteDeviceFormAdd', 'formSubmit_ajax', 'autositeRemoteDevice');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayAutositeRemoteDevice").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('autositeRemoteDeviceFormAddDis','/autositeRemoteDeviceAction.html', 'resultDisplayAutositeRemoteDevice', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="autositeRemoteDeviceFormAddDis" method="POST" action="/autositeRemoteDeviceAction.html" id="autositeRemoteDeviceFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Device Type</div>
        <input class="field" id="deviceType" type="text" size="70" name="deviceType" value="<%=WebUtil.display(_device_typeValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('autositeRemoteDeviceFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayAutositeRemoteDevice"></span>
<a href="javascript:backToXX('autositeRemoteDeviceFormAddDis','resultDisplayAutositeRemoteDevice')">show back</a><br>
<hr/>
