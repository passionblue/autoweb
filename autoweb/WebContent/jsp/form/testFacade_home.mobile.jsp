...
<script type="text/javascript" src="/testFacadeAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_test_facade_form.html?prv_returnPage=test_facade_home"> Add New </a> |
            <a href="v_test_facade_list.html?"> List Page </a> |
            <a href="v_test_facade_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/testFacadeAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/testFacadeAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/testFacadeAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/testFacadeAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/testFacadeAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/testFacadeAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/testFacadeAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="testFacadeForm_data_label" style="font-size: normal normal bold 10px verdana;" >Data </div>
    </td> 
    <td> 
	    <div id="testFacadeForm_facadeData_label" style="font-size: normal normal bold 10px verdana;" >Facade Data </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	<form name="testFacadeFormEditField_Data_<%=_TestFacade.getId()%>" method="get" action="/testFacadeAction.html" >


		<div id="testFacadeForm_data_field">
	    <div id="testFacadeForm_data_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="data" value="<%=WebUtil.display(_TestFacade.getData())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="data_<%= _TestFacade.getId()%>"><%=_TestFacade.getData() %></div>
            <a id="formSubmit" href="javascript:document.testFacadeFormEditField_Data_<%=_TestFacade.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_TestFacade.getId()%>', 'data', '<%=_TestFacade.getData()%>');">
	    	EditPrompt
	    	</a>
	...
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="test_facade_home">
	</form>
    ...
    <td> 
	<form name="testFacadeFormEditField_FacadeData_<%=_TestFacade.getId()%>" method="get" action="/testFacadeAction.html" >


		<div id="testFacadeForm_facadeData_field">
	    <div id="testFacadeForm_facadeData_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="facadeData" value="<%=WebUtil.display(_TestFacade.getFacadeData())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="facadeData_<%= _TestFacade.getId()%>"><%=_TestFacade.getFacadeData() %></div>
            <a id="formSubmit" href="javascript:document.testFacadeFormEditField_FacadeData_<%=_TestFacade.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_TestFacade.getId()%>', 'facadeData', '<%=_TestFacade.getFacadeData()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="facadeData">	Update </a><br>
			<a id="update_field_by_ajax_open_reply" href="#" rel="facadeData">	Update </a><br>
			<a id="update_field_by_ajax_get2field" href="#" rel="facadeData">	Update Test With Get2Field value </a><br>
    ...
    <td>
    <form name="testFacadeForm<%=_TestFacade.getId()%>2" method="get" action="/v_test_facade_form.html" >
        <a href="javascript:document.testFacadeForm<%=_TestFacade.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _TestFacade.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="test_facade_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_test_facade_form('<%=_TestFacade.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/testFacadeAction.html?del=true&id=<%=_TestFacade.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_test_facade('<%=_TestFacade.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_test_facade_form(target){
		location.href='/v_test_facade_form.html?cmd=edit&prv_returnPage=test_facade_home&id=' + target;
	}

	function confirm_test_facade(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_test_facade(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/testFacadeAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/testFacadeAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
	...
	$(document).ready(function(){
		...
		$("a#update_field_by_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/testFacadeAction.html?editfield=true&ajxr=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_open_reply").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testFacadeAction.html?editfield=true&ajaxRequest=true&ajaxOut=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_get2field").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/testFacadeAction.html?editfield=true&ajxr=get2field&id="+_id+"&"+_fieldName+"="+ _val,
				beforeSend: function(jqXHR, settings){
					//alert("sending");
				},
				complete: function(jqXHR, textStatus){
		...
		$("a#partition_test_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/partitionTo.html?fmt=getpart&partType=custom&partId=partition-test-facade",
		  		success: function(msg){ 
		     		alert(msg);
