...
<% 
    ...
    String _main_page_idValue= WebUtil.display((String)reqParams.get("mainPageId"));
    String _time_createdValue= WebUtil.display((String)reqParams.get("timeCreated"));
%> 

<a href="/v_section_home.html"> Section Home </a>
<%
	
	List list = null;
	list = SectionDS.getInstance().getBySiteId(site.getId());
...
%>
	...
	<div id="sectionFrame<%=_Section.getId() %>" >

		<div id="title" >
			title:<%= _Section.getTitle() %>
		</div>
		<div id="mainPageId" >
			mainPageId:<%= _Section.getMainPageId() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _Section.getTimeCreated() %>
		</div>
		<div>
		<a id="sectionDeleteButton" href="javascript:deleteThis('/sectionAction.html',<%= _Section.getId()%>,'sectionFrame<%=_Section.getId()%>' );" > 
...
<form name="sectionFormAdd" method="POST" action="/sectionAction.html" id="sectionFormAdd">
    ...
    <div id="sectionForm_mainPageId_text" class="formFieldText" >       
        <input class="field" id="_ffd_mainPageId" type="text" size="70" name="mainPageId" value="<%=WebUtil.display(_main_page_idValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="sectionFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/sectionAction.html', 'sectionFormAdd', 'formSubmit_ajax', 'section');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplaySection").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('sectionFormAddDis','/sectionAction.html', 'resultDisplaySection', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="sectionFormAddDis" method="POST" action="/sectionAction.html" id="sectionFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Main Page Id</div>
        <input class="field" id="mainPageId" type="text" size="70" name="mainPageId" value="<%=WebUtil.display(_main_page_idValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('sectionFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplaySection"></span>
<a href="javascript:backToXX('sectionFormAddDis','resultDisplaySection')">show back</a><br>
<hr/>
