...
<% 
    ...
    String _income_categoryValue= WebUtil.display((String)reqParams.get("incomeCategory"));
...
<a href="/v_chur_income_category_home.html"> ChurIncomeCategory Home </a>
<%
	
	List list = null;
	list = ChurIncomeCategoryDS.getInstance().getBySiteId(site.getId());

%>

<div id="churIncomeCategoryList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ChurIncomeCategory _ChurIncomeCategory = (ChurIncomeCategory) iter.next();	
%>

	<div id="churIncomeCategoryFrame<%=_ChurIncomeCategory.getId() %>" >

		<div id="incomeCategory" >
			incomeCategory:<%= _ChurIncomeCategory.getIncomeCategory() %>
		</div>
		<div id="display" >
			display:<%= _ChurIncomeCategory.getDisplay() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _ChurIncomeCategory.getTimeCreated() %>
		</div>
		<div>
		<a id="churIncomeCategoryDeleteButton" href="javascript:deleteThis('/churIncomeCategoryAction.html',<%= _ChurIncomeCategory.getId()%>,'churIncomeCategoryFrame<%=_ChurIncomeCategory.getId()%>' );" > 
...
<form name="churIncomeCategoryFormAdd" method="POST" action="/churIncomeCategoryAction.html" id="churIncomeCategoryFormAdd">
    ...
    <div id="churIncomeCategoryForm_incomeCategory_text" class="formFieldText" >       
        <input class="field" id="_ffd_incomeCategory" type="text" size="70" name="incomeCategory" value="<%=WebUtil.display(_income_categoryValue)%>"/> <span></span>
    </div>      
    ...
    <div id="churIncomeCategoryForm_display_text" class="formFieldText" >       
        ...
        <div id="churIncomeCategoryFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/churIncomeCategoryAction.html', 'churIncomeCategoryFormAdd', 'formSubmit_ajax', 'churIncomeCategory');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayChurIncomeCategory").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('churIncomeCategoryFormAddDis','/churIncomeCategoryAction.html', 'resultDisplayChurIncomeCategory', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="churIncomeCategoryFormAddDis" method="POST" action="/churIncomeCategoryAction.html" id="churIncomeCategoryFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Income Category</div>
        <input class="field" id="incomeCategory" type="text" size="70" name="incomeCategory" value="<%=WebUtil.display(_income_categoryValue)%>"/> <span></span>
		<br/>
	...
	<a href="javascript:clearFormXX('churIncomeCategoryFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayChurIncomeCategory"></span>
<a href="javascript:backToXX('churIncomeCategoryFormAddDis','resultDisplayChurIncomeCategory')">show back</a><br>
<hr/>
