...
<%
	Map itemizedExpenseTotal = ChurReportUtil.getYearlyTotalIncomeAmountByItem(site.getId(), _selectedYear);
    ...
    for(Iterator it = itemizedExpenseTotal.keySet().iterator();it.hasNext();) {
		...
		ChurIncomeItem item = ChurIncomeItemDS.getInstance().getById(itemKey.longValue());
		ChurIncomeCategory category = ChurIncomeCategoryDS.getInstance().getById(item.getCategoryId());
		
%>

	<tr>
		<td><%= category.getIncomeCategory()  %></td> 
