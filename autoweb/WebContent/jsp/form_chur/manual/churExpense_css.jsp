...
<%
	...
	_listChurExpenseItem_expenseItemId = SimpleSorterUtil.sortedByKey(_listChurExpenseItem_expenseItemId, new SorterActor<ChurExpenseItem>(){
	    public Object getSortable(ChurExpenseItem t){
	        return t.getExpenseItem();
	    }
	});
...
<%
	...
	List sortedPayeeList = SimpleSorterUtil.sortedByKey(_listChurPayee_payeeId, new SorterActor<ChurPayee>(){
	    public Object getSortable(ChurPayee t){
	        return t.getTitle();
	    }
	});

	for(Iterator iter = sortedPayeeList.iterator(); iter.hasNext();){
...
<%	} %>            
    ...
    <div id="churExpenseForm_checkNumber_text" class="formFieldText" >       
        <input id="checkNumber" autocomplete="off" class="field" type="text" size="10" name="checkNumber" value="<%=WebUtil.display(_check_numberValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
    ...
    <div id="churExpenseForm_amount_text" class="formFieldText" >       
        <input id="amount" autocomplete="off" class="field" type="text" size="20" name="amount" value="<%=WebUtil.display(_amountValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
    ...
    <div id="churExpenseForm_comment_text" class="formFieldText" >       
        <input id="comment" autocomplete="off" class="field" type="text" size="70" name="comment" value="<%=WebUtil.display(_commentValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
...
<%
    ...
    for(Iterator iter = list.iterator();iter.hasNext();) {
        ...
        if ( _oChurExpense.getYear() != _selectedYear ) continue;
        if ( !_oChurExpense.getWeek().equals(_selectedWeek)) continue;
        
    	totalWeekly +=  _oChurExpense.getAmount();   
...
<TR id="tableRow<%= _oChurExpense.getId()%>">
	...
	<td> <%= WebUtil.displayYesNo(_oChurExpense.getIsCash())  %> </td>
	<td> <%= _oChurExpense.getCheckNumber()  %> </td>
	<td> <%=  WebUtil.displayYesNo(_oChurExpense.getCheckCleared() ) %> </td>
...
Weekly Total:<%= WebUtil.displayMoney(totalWeekly)%> <br/> <br/>
