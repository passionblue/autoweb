...
<% 
	...
	_yearValue = ""+ChurWebUtil.getSelectedYear(request);
%> 
...
%>		
            <option value="<%=_obj.getId()%>" <%=HtmlUtil.getOptionSelect(_income_item_idValue, String.valueOf(_obj.getId()))%> ><%=_obj.getDisplay () + " " + _obj.getIncomeItem() + " (" + _obj.getId() + ")" %></option>
<%	} %>            
        </select>  <span></span>
    </div>      
	</div><div class="clear"></div>
...
<%
	...
	List list = ChurIncomeDS.getInstance().getBySiteIdWeekList(site.getId(), _selectedWeek);
