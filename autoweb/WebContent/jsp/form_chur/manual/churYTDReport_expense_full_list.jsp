...
<%
	...
	// Add wee by week because expenses could be added out of order. 
	List weeksOfYear = ChurUtil.getWeeksForYear(_selectedYear);
	List totalExpeseOfYear = new ArrayList();
	
	for(Iterator it = weeksOfYear.iterator();it.hasNext();) {
		String week = (String) it.next();
		
		List expensesForTheWeek = ChurExpenseDS.getInstance().getBySiteIdWeekList(site.getId(), week);

		totalExpeseOfYear.addAll(expensesForTheWeek);
	}
...
<%
	for(Iterator it = totalExpeseOfYear.iterator();it.hasNext();) {
		ChurExpense expense = (ChurExpense) it.next();
		ChurExpenseItem item = ChurExpenseItemDS.getInstance().getById(expense.getExpenseItemId());
		ChurExpenseCategory category = ChurExpenseCategoryDS.getInstance().getById(item.getCategoryId());
		
%>
	<tr>
		<td><%=expense.getWeek() %></td>
		<td><%=category.getExpenseCategory() %></td>
		<td><%= item.getExpenseItem() %></td>
		<td><%= expense.getAmount() %></td>
		<td><%= WebUtil.display(expense.getComment()) %> </td>
...
<TABLE id="table2" class="tablesorter" cellspacing="1">
    <thead>
        <tr> 
            <th>Category</th> 
            <th>Item</th> 
