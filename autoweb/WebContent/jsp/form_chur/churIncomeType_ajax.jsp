...
<% 
    ...
    String _income_typeValue= WebUtil.display((String)reqParams.get("incomeType"));
%> 

<a href="/v_chur_income_type_home.html"> ChurIncomeType Home </a>
<%
	
	List list = null;
	list = ChurIncomeTypeDS.getInstance().getBySiteId(site.getId());

%>

<div id="churIncomeTypeList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ChurIncomeType _ChurIncomeType = (ChurIncomeType) iter.next();	
%>

	<div id="churIncomeTypeFrame<%=_ChurIncomeType.getId() %>" >

		<div id="incomeType" >
			incomeType:<%= _ChurIncomeType.getIncomeType() %>
		</div>
		<div>
		<a id="churIncomeTypeDeleteButton" href="javascript:deleteThis('/churIncomeTypeAction.html',<%= _ChurIncomeType.getId()%>,'churIncomeTypeFrame<%=_ChurIncomeType.getId()%>' );" > 
...
<form name="churIncomeTypeFormAdd" method="POST" action="/churIncomeTypeAction.html" id="churIncomeTypeFormAdd">
    ...
    <div id="churIncomeTypeForm_incomeType_text" class="formFieldText" >       
        <input class="field" id="_ffd_incomeType" type="text" size="70" name="incomeType" value="<%=WebUtil.display(_income_typeValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="churIncomeTypeFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/churIncomeTypeAction.html', 'churIncomeTypeFormAdd', 'formSubmit_ajax', 'churIncomeType');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayChurIncomeType").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('churIncomeTypeFormAddDis','/churIncomeTypeAction.html', 'resultDisplayChurIncomeType', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="churIncomeTypeFormAddDis" method="POST" action="/churIncomeTypeAction.html" id="churIncomeTypeFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Income Type</div>
        <input class="field" id="incomeType" type="text" size="70" name="incomeType" value="<%=WebUtil.display(_income_typeValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('churIncomeTypeFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayChurIncomeType"></span>
<a href="javascript:backToXX('churIncomeTypeFormAddDis','resultDisplayChurIncomeType')">show back</a><br>
<hr/>
