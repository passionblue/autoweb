...
<% 
    ...
    String _remarkValue= WebUtil.display((String)reqParams.get("remark"));
%> 

<a href="/v_chur_payee_home.html"> ChurPayee Home </a>
<%
	
	List list = null;
	list = ChurPayeeDS.getInstance().getBySiteId(site.getId());

%>

<div id="churPayeeList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ChurPayee _ChurPayee = (ChurPayee) iter.next();	
%>

	<div id="churPayeeFrame<%=_ChurPayee.getId() %>" >

		<div id="title" >
			title:<%= _ChurPayee.getTitle() %>
		</div>
		<div id="remark" >
			remark:<%= _ChurPayee.getRemark() %>
		</div>
		<div>
		<a id="churPayeeDeleteButton" href="javascript:deleteThis('/churPayeeAction.html',<%= _ChurPayee.getId()%>,'churPayeeFrame<%=_ChurPayee.getId()%>' );" > 
...
<form name="churPayeeFormAdd" method="POST" action="/churPayeeAction.html" id="churPayeeFormAdd">
    ...
    <div id="churPayeeForm_remark_text" class="formFieldText" >       
        <input class="field" id="_ffd_remark" type="text" size="70" name="remark" value="<%=WebUtil.display(_remarkValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="churPayeeFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/churPayeeAction.html', 'churPayeeFormAdd', 'formSubmit_ajax', 'churPayee');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayChurPayee").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('churPayeeFormAddDis','/churPayeeAction.html', 'resultDisplayChurPayee', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="churPayeeFormAddDis" method="POST" action="/churPayeeAction.html" id="churPayeeFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Remark</div>
        <input class="field" id="remark" type="text" size="70" name="remark" value="<%=WebUtil.display(_remarkValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('churPayeeFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayChurPayee"></span>
<a href="javascript:backToXX('churPayeeFormAddDis','resultDisplayChurPayee')">show back</a><br>
<hr/>
