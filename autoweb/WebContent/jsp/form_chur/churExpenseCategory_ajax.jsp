...
<% 
    ...
    String _expense_categoryValue= WebUtil.display((String)reqParams.get("expenseCategory"));
    String _displayValue= WebUtil.display((String)reqParams.get("display"));
%> 

<a href="/v_chur_expense_category_home.html"> ChurExpenseCategory Home </a>
<%
	
	List list = null;
	list = ChurExpenseCategoryDS.getInstance().getBySiteId(site.getId());

%>

<div id="churExpenseCategoryList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ChurExpenseCategory _ChurExpenseCategory = (ChurExpenseCategory) iter.next();	
%>

	<div id="churExpenseCategoryFrame<%=_ChurExpenseCategory.getId() %>" >

		<div id="expenseCategory" >
			expenseCategory:<%= _ChurExpenseCategory.getExpenseCategory() %>
		</div>
		<div id="display" >
			display:<%= _ChurExpenseCategory.getDisplay() %>
		</div>
		<div>
		<a id="churExpenseCategoryDeleteButton" href="javascript:deleteThis('/churExpenseCategoryAction.html',<%= _ChurExpenseCategory.getId()%>,'churExpenseCategoryFrame<%=_ChurExpenseCategory.getId()%>' );" > 
...
<form name="churExpenseCategoryFormAdd" method="POST" action="/churExpenseCategoryAction.html" id="churExpenseCategoryFormAdd">
    ...
    <div id="churExpenseCategoryForm_expenseCategory_text" class="formFieldText" >       
        <input class="field" id="_ffd_expenseCategory" type="text" size="70" name="expenseCategory" value="<%=WebUtil.display(_expense_categoryValue)%>"/> <span></span>
    </div>      
    ...
    <div id="churExpenseCategoryForm_display_text" class="formFieldText" >       
        ...
        <div id="churExpenseCategoryFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/churExpenseCategoryAction.html', 'churExpenseCategoryFormAdd', 'formSubmit_ajax', 'churExpenseCategory');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayChurExpenseCategory").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('churExpenseCategoryFormAddDis','/churExpenseCategoryAction.html', 'resultDisplayChurExpenseCategory', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="churExpenseCategoryFormAddDis" method="POST" action="/churExpenseCategoryAction.html" id="churExpenseCategoryFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">


		 <div class="ajaxFormLabel"> Expense Category</div>
        <input class="field" id="expenseCategory" type="text" size="70" name="expenseCategory" value="<%=WebUtil.display(_expense_categoryValue)%>"/> <span></span>
		<br/>
	...
	<a href="javascript:clearFormXX('churExpenseCategoryFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayChurExpenseCategory"></span>
<a href="javascript:backToXX('churExpenseCategoryFormAddDis','resultDisplayChurExpenseCategory')">show back</a><br>
<hr/>
