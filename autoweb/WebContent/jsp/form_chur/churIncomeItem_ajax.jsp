...
<% 
    ...
    String _income_itemValue= WebUtil.display((String)reqParams.get("incomeItem"));
    String _displayValue= WebUtil.display((String)reqParams.get("display"));
%> 

<a href="/v_chur_income_item_home.html"> ChurIncomeItem Home </a>
<%
	
	List list = null;
	list = ChurIncomeItemDS.getInstance().getBySiteId(site.getId());

%>

<div id="churIncomeItemList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		ChurIncomeItem _ChurIncomeItem = (ChurIncomeItem) iter.next();	
%>

	<div id="churIncomeItemFrame<%=_ChurIncomeItem.getId() %>" >

		<div id="categoryId" >
			categoryId:<%= _ChurIncomeItem.getCategoryId() %>
		</div>
		<div id="incomeItem" >
			incomeItem:<%= _ChurIncomeItem.getIncomeItem() %>
		</div>
		<div id="display" >
			display:<%= _ChurIncomeItem.getDisplay() %>
		</div>
		<div>
		<a id="churIncomeItemDeleteButton" href="javascript:deleteThis('/churIncomeItemAction.html',<%= _ChurIncomeItem.getId()%>,'churIncomeItemFrame<%=_ChurIncomeItem.getId()%>' );" > 
...
<form name="churIncomeItemFormAdd" method="POST" action="/churIncomeItemAction.html" id="churIncomeItemFormAdd">
...
<%	} %>            
    ...
    <div id="churIncomeItemForm_incomeItem_text" class="formFieldText" >       
        <input class="field" id="_ffd_incomeItem" type="text" size="70" name="incomeItem" value="<%=WebUtil.display(_income_itemValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>
    ...
    <div id="churIncomeItemForm_display_text" class="formFieldText" >       
        ...
        <div id="churIncomeItemFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/churIncomeItemAction.html', 'churIncomeItemFormAdd', 'formSubmit_ajax', 'churIncomeItem');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayChurIncomeItem").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('churIncomeItemFormAddDis','/churIncomeItemAction.html', 'resultDisplayChurIncomeItem', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="churIncomeItemFormAddDis" method="POST" action="/churIncomeItemAction.html" id="churIncomeItemFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
        ...
        <option value="" >- Please Select -</option>
<%
	_listChurIncomeCategory_categoryId = ChurIncomeCategoryDS.getInstance().getBySiteId(site.getId());
	for(Iterator iter = _listChurIncomeCategory_categoryId.iterator(); iter.hasNext();){
		ChurIncomeCategory _obj = (ChurIncomeCategory) iter.next();
%>		
            <option value="<%=_obj.getId()%>" <%=HtmlUtil.getOptionSelect(_category_idValue, String.valueOf(_obj.getId()))%> ><%=_obj.getIncomeCategory() + " (" + _obj.getId() + ")" %></option>
<%	} %>            
		...
		<br/>
		 <div class="ajaxFormLabel"> Income Item</div>
        <input class="field" id="incomeItem" type="text" size="70" name="incomeItem" value="<%=WebUtil.display(_income_itemValue)%>"/> <span></span>
		<br/>
	...
	<a href="javascript:clearFormXX('churIncomeItemFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayChurIncomeItem"></span>
<a href="javascript:backToXX('churIncomeItemFormAddDis','resultDisplayChurIncomeItem')">show back</a><br>
<hr/>
