...
<% 
	...
	List ccList = ContentCategoryDS.getInstance().getBySiteId(site.getId());
%> 
...
<%
	for(Iterator iter = ccList.iterator(); iter.hasNext();){
		ContentCategory cc = (ContentCategory) iter.next();
		Page p = PageDS.getInstance().getById(cc.getPageId());
%>
		---------------------
		<%= cc.getId() %> <%= cc.getCategory() %><br/>
		<%= p.getPageName() %><%= p.getId() %><br/>
<%
	}
%>