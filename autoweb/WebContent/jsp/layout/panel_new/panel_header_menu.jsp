...
<% } %>
<div id="panel-container<%="-"+panel.getId()%>" style="display:block" class="panel-container" ><!-- panel_new/panel_header_menu -->
<div id="header-menu<%=panelIdSuffix%>" class="header-menu" >
	<ul id="nav<%=panelIdSuffix%>" style="overflow: auto;" class="nav-list">
	<%
		...
		for(Iterator iter = menuItems.iterator();iter.hasNext();) {
			...
			if (mi.getTargetType() == MenuItemUtil.TYPE_PAGE){ 
			    ...
			    JspUtil.getLogger().debug("Menu Item for " + panel.getId() + " : " + " page " + p.getPageMenuTitle());
...
//					continue;
	...
	%>
		<li id="nav-item<%=panelIdSuffix%>" rel="<%=mi.getId() %>"  class="nav-item">
