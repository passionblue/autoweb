...
<%
	...
	boolean closed = PollUtil.pollClosed(poll);
...
%>
	...
	<div id="ans-<%=poll.getId()%>-<%=ans.getAnswerNum() %>" class="pollDispAnswer">
		<div class="pollDispAnswerControl">
<%
		if ( WebParamUtil.getBooleanValue(poll.getAllowMultiple())) {
%>
		<input id="ans-<%=poll.getId()%>-<%=ans.getAnswerNum() %>-input" type="checkbox" name="answer_<%=WebUtil.isTrue(ans.getOwnAnswer())?-1:ans.getAnswerNum()%>" /> 
<%
		} else {
%>
		<input id="ans-<%=poll.getId()%>-<%=ans.getAnswerNum() %>-input" type="radio" name="answer" value="<%=WebUtil.isTrue(ans.getOwnAnswer())?-1:ans.getAnswerNum()%>" /> 
<%	    
		}
%>
		</div><!-- pollDispAnswerControl -->
		<div id="ans-<%=poll.getId()%>-<%=ans.getAnswerNum() %>-input" class="pollDispAnswerText">
<%
		if (WebUtil.isNotNull(ans.getImageUrl())){
%>		
		<img height="100" width="100"  src="<%=ans.getImageUrl() %>" style="float:left;margin-right: 10px;"/>	
<%
		}
%>			
		<%= ans.getText() %>
<%		if ( WebUtil.isTrue(poll.getAllowOwnAnswer()) && WebUtil.isTrue(ans.getOwnAnswer())){ %>		
		<br/><input id="ownAnswer<%=poll.getId() %>" type="text" size="50" name="ownAnswer" value="" />
<%		} %>		

		</div><div class="clear"></div><!-- pollDispAnswerText -->
...
<%
	if (closed){
%>
	<p style="margin-left: 10px;font: normal normal bold 10px verdana"> *This Poll Closed.</p>
<%	}%>


<div class="pollDispSubmitFrame">
<%
...
</div> <!-- pollDispSubmitFrame -->
</div> <!-- pollDispFrameInside -->
<div class="clear"></div>
