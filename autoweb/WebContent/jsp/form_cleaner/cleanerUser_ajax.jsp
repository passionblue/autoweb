...
<% 
    ...
    String _inactivatedValue= WebUtil.display((String)reqParams.get("inactivated"));
...
<a href="/v_cleaner_user_home.html"> CleanerUser Home </a>
<%
	
	List list = null;
	list = CleanerUserDS.getInstance().getBySiteId(site.getId());

%>

<div id="cleanerUserList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		CleanerUser _CleanerUser = (CleanerUser) iter.next();	
%>

	<div id="cleanerUserFrame<%=_CleanerUser.getId() %>" >

		<div id="autositeUserId" >
			autositeUserId:<%= _CleanerUser.getAutositeUserId() %>
		</div>
		<div id="inactivated" >
			inactivated:<%= _CleanerUser.getInactivated() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _CleanerUser.getTimeCreated() %>
		</div>
		<div id="timeUpdated" >
			timeUpdated:<%= _CleanerUser.getTimeUpdated() %>
		</div>
		<div>
		<a id="cleanerUserDeleteButton" href="javascript:deleteThis('/cleanerUserAction.html',<%= _CleanerUser.getId()%>,'cleanerUserFrame<%=_CleanerUser.getId()%>' );" > 
...
<form name="cleanerUserFormAdd" method="POST" action="/cleanerUserAction.html" id="cleanerUserFormAdd">
    ...
    <div id="cleanerUserForm_inactivated_text" class="formFieldText" >       
        <input class="field" id="_ffd_inactivated" type="text" size="70" name="inactivated" value="<%=WebUtil.display(_inactivatedValue)%>"/> <span></span>
    </div>      
	</div><div class="clear"></div>

        <div id="cleanerUserFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/cleanerUserAction.html', 'cleanerUserFormAdd', 'formSubmit_ajax', 'cleanerUser');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayCleanerUser").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('cleanerUserFormAddDis','/cleanerUserAction.html', 'resultDisplayCleanerUser', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="cleanerUserFormAddDis" method="POST" action="/cleanerUserAction.html" id="cleanerUserFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
		...
		<br/>
		 <div class="ajaxFormLabel"> Inactivated</div>
        <input class="field" id="inactivated" type="text" size="70" name="inactivated" value="<%=WebUtil.display(_inactivatedValue)%>"/> <span></span>
		<br/>

	<a id="formSubmit_ajax_simpleform" href="javascript:sendRequest()">Submit</a><br>
	<a href="javascript:clearFormXX('cleanerUserFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayCleanerUser"></span>
<a href="javascript:backToXX('cleanerUserFormAddDis','resultDisplayCleanerUser')">show back</a><br>
<hr/>
