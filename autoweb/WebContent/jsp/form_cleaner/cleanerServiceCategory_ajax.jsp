...
<a href="/v_cleaner_service_category_home.html"> CleanerServiceCategory Home </a>
<%
	
	List list = null;
	list = CleanerServiceCategoryDS.getInstance().getBySiteId(site.getId());

%>

<div id="cleanerServiceCategoryList"> 
<%
	for(Iterator iter = list.iterator();iter.hasNext();){
		CleanerServiceCategory _CleanerServiceCategory = (CleanerServiceCategory) iter.next();	
%>

	<div id="cleanerServiceCategoryFrame<%=_CleanerServiceCategory.getId() %>" >

		<div id="title" >
			title:<%= _CleanerServiceCategory.getTitle() %>
		</div>
		<div id="imagePath" >
			imagePath:<%= _CleanerServiceCategory.getImagePath() %>
		</div>
		<div id="imagePathLocal" >
			imagePathLocal:<%= _CleanerServiceCategory.getImagePathLocal() %>
		</div>
		<div>
		<a id="cleanerServiceCategoryDeleteButton" href="javascript:deleteThis('/cleanerServiceCategoryAction.html',<%= _CleanerServiceCategory.getId()%>,'cleanerServiceCategoryFrame<%=_CleanerServiceCategory.getId()%>' );" > 
...
<form name="cleanerServiceCategoryFormAdd" method="POST" action="/cleanerServiceCategoryAction.html" id="cleanerServiceCategoryFormAdd">
    ...
    <div id="cleanerServiceCategoryForm_imagePathLocal_text" class="formFieldText" >       
        ...
        <div id="cleanerServiceCategoryFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/cleanerServiceCategoryAction.html', 'cleanerServiceCategoryFormAdd', 'formSubmit_ajax', 'cleanerServiceCategory');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayCleanerServiceCategory").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('cleanerServiceCategoryFormAddDis','/cleanerServiceCategoryAction.html', 'resultDisplayCleanerServiceCategory', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="cleanerServiceCategoryFormAddDis" method="POST" action="/cleanerServiceCategoryAction.html" id="cleanerServiceCategoryFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
	...
	<a href="javascript:clearFormXX('cleanerServiceCategoryFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayCleanerServiceCategory"></span>
<a href="javascript:backToXX('cleanerServiceCategoryFormAddDis','resultDisplayCleanerServiceCategory')">show back</a><br>
<hr/>
