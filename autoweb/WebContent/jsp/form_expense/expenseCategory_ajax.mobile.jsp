...
<a href="/v_expense_category_home.html"> ExpenseCategory Home </a>
<%
	
	List list = null;
	list = ExpenseCategoryDS.getInstance().getBySiteId(site.getId());
...
%>
	...
	<div id="expenseCategoryFrame<%=_ExpenseCategory.getId() %>" >

		<div id="expenseCategory" >
			expenseCategory:<%= _ExpenseCategory.getExpenseCategory() %>
		</div>
		<div id="timeCreated" >
			timeCreated:<%= _ExpenseCategory.getTimeCreated() %>
		</div>
		<div id="timeUpdated" >
			timeUpdated:<%= _ExpenseCategory.getTimeUpdated() %>
		</div>
		<div>
		<a id="expenseCategoryDeleteButton" href="javascript:deleteThis('/expenseCategoryAction.html',<%= _ExpenseCategory.getId()%>,'expenseCategoryFrame<%=_ExpenseCategory.getId()%>' );" > 
...
<form name="expenseCategoryFormAdd" method="POST" action="/expenseCategoryAction.html" id="expenseCategoryFormAdd">
    ...
    <div id="expenseCategoryForm_expenseCategory_text" class="formFieldText" >       
        ...
        <div id="expenseCategoryFormEdit_submit" class="formSubmit" >       
            <a id="formSubmit_ajax" href="javascript:sendFormAjax2('/expenseCategoryAction.html', 'expenseCategoryFormAdd', 'formSubmit_ajax', 'expenseCategory');">Submit</a>
        </div> 
...
function responseCallback(data){
	document.getElementById("resultDisplayExpenseCategory").innerHTML = data;
}

function sendRequest(){
	xmlhttpPostXX('expenseCategoryFormAddDis','/expenseCategoryAction.html', 'resultDisplayExpenseCategory', '${ajax_response_fields}', responseCallback);
}
</script>



<form name="expenseCategoryFormAddDis" method="POST" action="/expenseCategoryAction.html" id="expenseCategoryFormAddDis">
	<INPUT TYPE="HIDDEN" NAME="add" value="true">
	...
	<a href="javascript:clearFormXX('expenseCategoryFormAddDis')">Clear Form</a><br>
    <a href="/moveTo.html?dest=<%=cancelPage%>">Cancel</a><br>

	<!--INPUT TYPE="HIDDEN" id="_ffd_wpid" name="wpid" value=" _wpId " -->

</form>
<span id="resultDisplayExpenseCategory"></span>
<a href="javascript:backToXX('expenseCategoryFormAddDis','resultDisplayExpenseCategory')">show back</a><br>
<hr/>
