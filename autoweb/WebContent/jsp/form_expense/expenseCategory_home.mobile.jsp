...
<script type="text/javascript" src="/expenseCategoryAction.html?ajxr=getscriptform"></script>
...
<TABLE border="1" width="100%" bgcolor="transparent" align="center" cellspacing="0" style="border-width: 1px; border-style: solid">
    <TR>
        <TD>
            <a href="v_expense_category_form.html?prv_returnPage=expense_category_home"> Add New </a> |
            <a href="v_expense_category_list.html?"> List Page </a> |
            <a href="v_expense_category_ajax.html?"> Ajax Sampler </a> |
        </TD>
    </TR>
    <TR>
        <TD>
        	AJAX | 
			<a href="/expenseCategoryAction.html?ajxr=getmodalform&formfieldlist=&forcehiddenlist=" rel="facebox"> open form (custom field list)</a> |
			<a href="/expenseCategoryAction.html?ajxr=getmodalform" 			rel="facebox"> open form</a> |
			<a href="/expenseCategoryAction.html?ajxr=getlisthtml"  			rel="facebox"> getlisthtml</a> |
			<a href="/expenseCategoryAction.html?ajxr=getlistjson"  			rel="facebox"> getlistjson</a> |
			<a href="/expenseCategoryAction.html?ajxr=getjson&ajaxOutArg=first" rel="facebox"> getjson first</a> |
			<a href="/expenseCategoryAction.html?ajxr=getjson&ajaxOutArg=last" 	rel="facebox"> getjson last</a> |
			<a href="/expenseCategoryAction.html?ajxr=getlistdata" 				rel="facebox"> getlistdata</a> |
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> 
	    <div id="expenseCategoryForm_expenseCategory_label" style="font-size: normal normal bold 10px verdana;" >Expense Category </div>
    </td> 
	<td></td>
	<td></td>
</TR>
...
<%
    ...
    for(Iterator iter = list.iterator();iter.hasNext();) {
        ExpenseCategory _ExpenseCategory = (ExpenseCategory) iter.next();
		//TODO 
        fieldString += "\"" +  _ExpenseCategory.getId() + "\",";
%>
...
<TR bgcolor="#ffffff" valign="top">
    ...
    <td> <%= _ExpenseCategory.getId() %> </td>


    <td> 
	<form name="expenseCategoryFormEditField_ExpenseCategory_<%=_ExpenseCategory.getId()%>" method="get" action="/expenseCategoryAction.html" >


		<div id="expenseCategoryForm_expenseCategory_field">
	    <div id="expenseCategoryForm_expenseCategory_text" class="formFieldText" >       
	        <input id="field" type="text" size="20" name="expenseCategory" value="<%=WebUtil.display(_ExpenseCategory.getExpenseCategory())%>"/> <span></span>
	    </div>      
		</div><div class="clear"></div>

		<br>
			<div id="expenseCategory_<%= _ExpenseCategory.getId()%>"><%=_ExpenseCategory.getExpenseCategory() %></div>
            <a id="formSubmit" href="javascript:document.expenseCategoryFormEditField_ExpenseCategory_<%=_ExpenseCategory.getId()%>.submit();">Submit</a>
            <br>
		    <a href="javascript:;" title="UpdatePrompt" class="deleteLink" onclick="update_cleaner_pickup_field_dialog('<%=_ExpenseCategory.getId()%>', 'expenseCategory', '<%=_ExpenseCategory.getExpenseCategory()%>');">
	    	EditPrompt
	    	</a>
			<br> <!-- update by ajax call -->
			<a id="update_field_by_ajax" href="#" rel="expenseCategory">	Update </a><br>
			<a id="update_field_by_ajax_open_reply" href="#" rel="expenseCategory">	Update </a><br>
			<a id="update_field_by_ajax_get2field" href="#" rel="expenseCategory">	Update Test With Get2Field value </a><br>

	<INPUT TYPE="HIDDEN" NAME="editfield" value="true">
	<INPUT TYPE="HIDDEN" NAME="id" value="<%=_ExpenseCategory.getId()%>">
	<INPUT TYPE="HIDDEN" NAME="returnPage" value="expense_category_home">
	</form>
    ...
    <td>
    <form name="expenseCategoryForm<%=_ExpenseCategory.getId()%>2" method="get" action="/v_expense_category_form.html" >
        <a href="javascript:document.expenseCategoryForm<%=_ExpenseCategory.getId()%>2.submit();">Edit</a>           
        <INPUT TYPE="HIDDEN" NAME="id" value="<%= _ExpenseCategory.getId() %>">
        <INPUT TYPE="HIDDEN" NAME="cmd" value="edit">
        <INPUT TYPE="HIDDEN" NAME="prv_returnPage" value="expense_category_home">
    </form>
    <a href="javascript:;" title="Edit" class="deleteLink" onclick="edit_expense_category_form('<%=_ExpenseCategory.getId()%>');">Edit</a>
    </td>

    <td>
    <a href="/expenseCategoryAction.html?del=true&id=<%=_ExpenseCategory.getId()%>"> Delete </a><BR>
    <a href="javascript:;" title="Delete User" class="deleteLink" onclick="confirm_remove_expense_category('<%=_ExpenseCategory.getId()%>');">DeleteWConfirm</a>
    </td>
</TR>
...
<script type="text/javascript">
	...
	function edit_expense_category_form(target){
		location.href='/v_expense_category_form.html?cmd=edit&prv_returnPage=expense_category_home&id=' + target;
	}

	function confirm_expense_category(target, txt){
		$ .prompt(txt,{ 
	...
	function confirm_remove_expense_category(target){
		$ .prompt('Are you sure you want to remove this?',{ 
			buttons:{Delete:true, Cancel:false},
			callback: function(v,m,f){
				if(v){
					location.href="/expenseCategoryAction.html?del=true&id="+target;
				}
			}
		});
	}
			...
			callback: function(v,m,f){
				if (v){
					if (f.myname == "") {
						...
						location.href="/expenseCategoryAction.html?editfield=true&returnPage=cleaner_pickup_home&id="+targetId+"&"+targetField +"=" +f.myname;
					}
				}
				return true;
			}
	...
	$(document).ready(function(){
		...
		$("a#update_field_by_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/expenseCategoryAction.html?editfield=true&ajxr=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_open_reply").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/expenseCategoryAction.html?editfield=true&ajaxRequest=true&ajaxOut=getfieldbyname&id="+_id+"&"+_fieldName+"="+ _val+"&fieldname=" + _fieldName,
		...
		$("a#update_field_by_ajax_get2field").click(function () {
	    	...
	    	$ .ajax({
			   type: "GET",
		   		url: "/expenseCategoryAction.html?editfield=true&ajxr=get2field&id="+_id+"&"+_fieldName+"="+ _val,
				beforeSend: function(jqXHR, settings){
					//alert("sending");
				},
				complete: function(jqXHR, textStatus){
		...
		$("a#partition_test_ajax").click(function () {
			...
			$ .ajax({
			   type: "GET",
		   		url: "/partitionTo.html?fmt=getpart&partType=custom&partId=partition-expense-category",
		  		success: function(msg){ 
		     		alert(msg);
